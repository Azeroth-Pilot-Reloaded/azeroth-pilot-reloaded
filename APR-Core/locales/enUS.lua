local app = select(2, ...);

app.L = {
	["ACCEPT_Q"] = "Accept the quest(s)",
	["ACTIVATE_SCANNER"] = "Activate the scanner",
	["AFK"] = "AFK",
	["AID_GUARDIAN"] = "Aid the Guardian",
	["ALL_Q"] = "All quest(s)",
	["APPRENTICE_RIDING"] = "Apprentice Riding",
	["ARCANE_DEVICES_CAN_BE_SEEN_ON_MINIMAP"] = "Arcane devices can be nseen on minimap.",
	["ARROW_OPTION"] = "Arrow Options",
	["ARROW_SCALE"] = "Arrow Scale",
	["ASSULT_SKIP"] = "The NPC won't be there if an assault is active! If so, skip ahead",
	["AT_FLIGHTPATH"] = "At the flightpath",
	["AT_GAZLOWE"] = "At Gazlowe",
	["AUTO_PATH_HELPER"] = "Auto Path Helper",
	["AUTO_PICK_REWARD_ITEM"] = "Auto-pick the quest reward by iLvl",
	["AUTO_REPAIR"] = "Auto-repair",
	["AUTO_SELECTION_OF_DIALOG"] = "Auto-selection of dialog",
	["AUTO_VENDOR"] = "Auto-sell gray items",
	["BANNER_IN_HUTS"] = "The banner is in one of the huts",
	["BENEATH_HANDIN"] = "Re-enter into the area if you can't turn-in the quest(s)",
	["BEWARE_TWO_LEVEL"] = "Be aware of the two level \"??'s\"",
	["BLOOD"] = "Blood",
	["BLOODLUST"] = "It's a good time to Bloodlust",
	["BONUS_OBJECTIVE"] = "Bonus Objective",
	["BONUS_OBJECTIVE_THIEVING_GNOLLS"] = "Bonus Objective: Thieving Gnolls",
	["BOTTOM_CAVE"] = "Go to the bottom of the Fissure of Fury",
	["BREWERY"] = "Brewery",
	["BUILD_BOAT"] = "Build the boat",
	["BUILD_CAMPFIRE"] = "Build the campfire",
	["BURN_INCENSE"] = "Burn the incense",
	["BUY_7_FRIGIDFISH"] = "Buy 7 Frigidfish",
	["BUY_BOTTLE_OF_GROG"] = "Buy a Bottle of Grog from Daisy",
	["BUY_DICED_MEAT"] = "Buy 'Diced Meat'",
	["BUY_ELYSIAN_THREAD"] = "Buy an Elysian Thread from Caretaker Mirene, close to Klystere",
	["BUY_MALT_OFF_INNKEEPER"] = "Buy a Rhapsody Malt from Innkeeper Belm",
	["BY_ROCK"] = "Go by the rock",
	["CAGED_BAKARS_AND_BAKAR_COLLARS_ARE_MARKED_ON_YOUR_MINIMAP"] = "Caged Bakars and Bakar Collars are marked on your minimap.",
	["CALL_BOARD"] = "Go to the Heroes Call board",
	["CANNERY_BAG_DISGUISE"] = "Open the Cannary's Cache and use the Clever Plant Disguise Kit and Tagg Mosshide",
	["CARRY_EGG_SAFETY_DO_NOT_GET_ON_YOUR_MOUNT_OR_YOU_WILL_DROP_THE_EGG"] = "Carry the egg to safety. Do not get on your mount or you will drop the egg.",
	["CAUTERIZE_WOUND"] = "Cauterize the wound",
	["CAVES_AROUND_TREE"] = "Go through the caves around the big tree",
	["CHEST_DOWN_HOLE"] = "Grab the chest down the hole",
	["CHEST_INSIDE_BUILDING"] = "Grab the chest inside the building",
	["CHEST_INSIDE_HUT"] = "Grab the chest inside the hut",
	["CHOICES_DONT_MATTER"] = "Choices dont matter",
	["CHOOSE_WHY_ARENT_DRAGONS_HERE_MEET_US"] = "Choose: Why aren't the dragons here to meet us?",
	["CLEAN_SHRINE"] = "Clean the shrine",
	["CLICK_BOOK_CHOICES_DONT_MATTER"] = "Click the book, choices don't matter",
	["CLICK_BRAZIER"] = "Click the brazier",
	["CLICK_BUFFS_IN_ZONE"] = "There's several clickable buffs around the zone",
	["CLICK_DOG"] = "Click the dog",
	["CLICK_ETERNAL_FLAME"] = "Click the Eternal Flame",
	["CLICK_EXTRAACTIONBUTTON"] = "Click ExtraActionButton",
	["CLICK_GONG"] = "Click on the Challenge Gong",
	["CLICK_INCENSE"] = "Click the incense on ground",
	["CLICK_ON_NPC"] = "Click on Zen'Kiki so he pulls the Captive Plaguebears",
	["CLICK_ROSTRUM_OF_TRANSFORMATION"] = "Click the Rostrum of Transformation",
	["CLICK_SHACKLE"] = "Click the shackles",
	["CLICK_STONE_TABLET"] = "Click the stone tablet",
	["CLICK_SURVEYORS_DISC"] = "Click Surveyor's Disc",
	["CLICK_THE_VISION"] = "Click the Vision of Sailor's Memory",
	["CLOSE"] = "Close",
	["CLOSE_JAINA"] = "Stay close to Jaina's proximity, or she won't move",
	["CLOSEST_FP"] = "Go to the closest flight point",
	["CLUES_CAN_BE_SEEN_ON_YOUR_MINIMAP"] = "Clues can be seen on your minimap",
	["COLLECT_WOOD"] = "Collect wood",
	["COMPLETE_Q"] = "Complete the quest(s)",
	["COMPLETES_ONCE_YOU_REACH_CHROMIE"] = "Completes once you reach Chromie",
	["COMPLETES_WHEN_YOU_ARE_CLOSE_BUBBLE"] = "Completes when you are close to the bubble",
	["COMPLETES_WHEN_YOU_ENTER_BUBBLE"] = "Completes when you enter bubble",
	["COMPLETES_WHEN_YOU_GET_CLOSE"] = "Completes when you get close",
	["COMPLETES_WHEN_YOU_GET_CLOSER_THRONE"] = "Completes when you get closer to the throne",
	["COMPLETES_WHEN_YOU_TALK_NPC"] = "Completes when you talk to NPC",
	["CONSULT_KAVIA"] = "Consult Kavia",
	["CONSULT_LEFT"] = "Consult left",
	["CONSULT_RIGHT"] = "Consult right",
	["CUSTOM_PATH"] = "Custom Path",
	["DAGGER_DOOR"] = "Get the Shadowmoon Sacrificial Dagger by the door",
	["DALARAN_CRATER_PORTAL"] = "Use the Dalaran Crater portal",
	["DECLINE_Q"] = "Decline the quest(s)",
	["DEMENTORS_DROPS_BOOKS"] = "Shadowborne Dementor's drop Shadow Council Spellbooks",
	["DESTINATION"] = "Destination",
	["DESTROY_PILLAR_INSIDE_THE_CAVE"] = "Destroy the pillar inside the cave",
	["DETACH_Q_ITEM_BTN"] = "Detatch the Quest Item buttons",
	["DISABLE_HEIRLOOM_WARNING"] = "Disable Heirloom Warning",
	["DISABLED_ADDON_LIST"] = "is disabled in your Add-on list",
	["DISPELL_STORMS"] = "Dispell the storms",
	["DISPELL_TOY_BOAT"] = "Dispell the toy boat",
	["DO_BONUS_OBJECTIVE"] = "Do the Bonus Objective",
	["DO_NOT_LOOT_HYDRA"] = "Do not loot the Devouring Hydra's until you have skinned them, or they will dissapear",
	["DO_NOT_USE_GLIDER"] = "Do not use gliders",
	["DOING_EMOTE"] = "Do the emote",
	["DOTS_EXPIRE"] = "Let your debuffs expire before clicking the Shrine of Thunder",
	["DOWN_ELEVATOR"] = "Go down the elevator",
	["DOWNSTAIRS"] = "Downstairs",
	["DRAGON_IS_FLYING_ABOVE_AREA_POINT_THE_CAMERA_AT_DRAGON"] = "Dragon is flying above the area; point the camera at dragon",
	["DRAGONRIDE_BACK_TOWARDS_RUBY_LIFESHRINE"] = "Dragonride back towards the Ruby Lifeshrine",
	["DROPS_FROM_THISTLEFURS"] = "Troll Charm's drop from Thistlefur Avengers",
	["EAGLE_FEATHER"] = "Eagle Feather",
	["EMPTY_GREAT_HALL"] = "Go into the empty lot to the right of the Great Hall",
	["ENTER_CITADEL"] = "Enter the citadel",
	["ENTER_DISC"] = "Enter the Disc",
	["ENTER_MIRROR"] = "Enter the mirror",
	["ENTER_MOUNTAIN"] = "Enter the mountain",
	["ENTER_PLANE"] = "Enter the plane",
	["ENTER_PORTAL"] = "Enter the portal",
	["EQUIP_GOGGLES"] = "Equip the Goggles of Gem Hunting and find Elven Gems",
	["ERROR"] = "Error",
	["EXAMINE_BODIES_MARKED_ON_YOUR_MAP"] = "Examine bodies marked on your map",
	["EXAMINE_FLASK"] = "Examine Flask",
	["EXAMINE_TEAPOT"] = "Examine Teapot",
	["EXAMINE_TOY"] = "Examine Toy",
	["EXIT_CAVE"] = "Exit the cave",
	["EXPERT_RIDING"] = "Expert Riding",
	["EXTRA"] = "Extra",
	["EXTRA_ACTION_BUTTON_NOT_NEEDED"] = "You don't need to use the extra action button",
	["FALSEBOTTOMED_JAR"] = "Find the False Jar",
	["FILL_VIAL"] = "Fill the Empty Moonwell Vial",
	["FIND_CURIER"] = "Find the Forsaken Courier along the road",
	["FIRAVA_REKINDLER"] = "Firava the Rekindler",
	["FIRST_CLUE"] = "First clue",
	["FIRST_HUNT"] = "First Hunt",
	["FISH_1"] = "Fish 1",
	["FISH_2__3"] = "Fish 2 & 3",
	["FISHES_CAN_BE_SEEN_ON_MINIMAP"] = "Fishes can be seen on the minimap",
	["FLY_BACK"] = "Fly back",
	["FLY_BACK_OVER"] = "Fly back over",
	["FLY_GREAT_SEAL"] = "Fly to the Great Seal",
	["FLY_GUARDIAN_VELOMIR"] = "Fly to Guardian Velomir",
	["FLY_NISHA"] = "Fly with Nisha",
	["FLY_OVER_MOUNTAIN"] = "Fly over the mountain",
	["FLY_TO"] = "Fly to",
	["FLY_WITH_YOUR_DRAGON_AVOID_BOMBS"] = "Fly with your dragon to avoid bombs",
	["FLYING_AROUND_THE_FOREST"] = "Stinglasher is flying around the forest",
	["FOLLOW_SENDRAX_CLOSELY"] = "Follow Sendrax closely",
	["FOLLOW_WAGON_CLOSELY_AND_KILL_MOBS"] = "Follow the wagon closely and kill mobs",
	["FOLLOW_ZOLANI"] = "Stay close to Zolani's proximity, or she won't move",
	["FOURTH_CLUE"] = "Fourth clue",
	["FREEZING_WATERS_CAN_BE_SEEN_ON_MINIMAP"] = "Freezing Waters can be seen on minimap",
	["FRESHLEAF_BUFF"] = "Freshleaf buff",
	["FUNERAL_OFFERINGS_ARE_MARKED_ON_MINIMAP"] = "Funeral Offerings are marked on minimap",
	["GAZLOWE_PORTAL"] = "Wait for Gazlowe to exit the portal to turn-in the quest(s)",
	["GENERAL_OPTION"] = "General Options",
	["GET_CROSSBOW"] = "Get Crossbow",
	["GET_CROWBAR"] = "Get Crowbar from Treasure hunters",
	["GET_DRAGON_GLYPH_FLOATING_IN_SKY_BY_FLYING_THROUGH_IT"] = "Get Dragon Glyph floating in the sky by flying through it",
	["GET_EGG"] = "Get Egg",
	["GET_FLIGHTPATH"] = "Get Flight Point",
	["GET_INSIDE"] = "Get inside",
	["GET_INSIDE_AND_USE_PORTAL_TOP"] = "Get inside and use portal to the top",
	["GET_JOURNEYMAN_RIDING"] = "Get Journeyman Riding",
	["GET_KEY_CAVE"] = "Get the key in the cave",
	["GET_ON_DRAGONMOUNT_HOP_OVER_SIDE_GLIDE_DOWN_COMMANDER_LETHANAK"] = "Get on dragonmount, hop over side, glide down to Commander Lethanak",
	["GET_ON_ROCK"] = "Get on the rock",
	["GET_QUEST_FROM_BOOK_ON_ROCK"] = "Get quest from book on rock",
	["GET_TREASURE"] = "Get Treasure",
	["GET_WEAPONS_CACHE"] = "Get Weapons Cache",
	["GIVERS_AROUND_AREA"] = "The Quest givers wander around the area",
	["GO_BACK_DOWN_HILL"] = "Go back down the hill",
	["GO_BLASTEDLANDS"] = "Go To BlastedLands",
	["GO_CAVE"] = "Go to the Fissure of Fury",
	["GO_EAST"] = "go East",
	["GO_INSIDE_CAVE"] = "Go inside the cave",
	["GO_INSIDE_TUNNEL"] = "Go inside the tunnel",
	["GO_INTO_HUT"] = "Go into the Hut",
	["GO_NORTH"] = "Go North",
	["GO_NORTH_WEST_GARRISON"] = "Go North West of Garrison",
	["GO_NORTHWEST"] = "Go Northwest",
	["GO_ON_TOP_OF_ARCHIVES"] = "Go on top of Archives",
	["GO_PORTAL_ROOM"] = "Go to Portal Room",
	["GO_SINFALL"] = "Go Into Sinfall",
	["GO_SOUTH"] = "Go South",
	["GO_TO"] = "Go To",
	["GO_WEST"] = "go West",
	["GOHEAD_NO_RP"] = "You can run ahead if you don't want to RP walk",
	["GORLASH_PATROLLS"] = "Gorlash patrolls",
	["GORMLINGS_COLLECTED"] = "gormlings collected",
	["GOSSAMER_THREAD_BUFF"] = "Gossamer Thread buff",
	["GRAB_EGG"] = "Grab the egg",
	["GRAB_HEAL_DMG_BUFF"] = "Grab the healing haste and damage buffs (Big Circles)",
	["GRAB_WEAPON_ON_GROUND"] = "Grab your weapon on the ground when you're disarmed!",
	["GROUP"] = "Group",
	["GROUP_Q"] = "Ask for group quest",
	["HALFWAY_UP_CLIFF"] = "Halfway up the cliff wall",
	["HAMMER_ON_GONGS"] = "Hurl Kyrian Hammer on the gongs!",
	["HANDIN_NEWS_TALADOR"] = "Handin news from Talador",
	["HANGING_UNDERNEATH_CLIFF"] = "Hanging underneath cliff",
	["HARP_TRAP"] = "Suspiciously Untouched Harp",
	["HE_PATROLS"] = "He Patrols",
	["HEAD_CAVE"] = "Head to the cave",
	["HEAD_SOUTHEAST"] = "Head Southeast",
	["HEAD_WYVERNS_TAIL_INN"] = "Head to Wyvern's Tail Inn",
	["HEAD_ZEPPELIN_TOWER"] = "Head to Zeppelin Tower",
	["HERB_BAG_DOWNSTAIRS"] = "Herb bag downstairs",
	["HIDE_RIDING"] = "Hide mount skill learning messages",
	["HOP_ON_RELASTRASZA"] = "Hop on Relastrasza",
	["HOP_ON_RUBY_WHELPLING"] = "Hop on Ruby Whelpling",
	["HORN_DOWNSTAIRS"] = "Horn downstairs",
	["HORN_ON_THE_GROUND"] = "When he stops taking damage click on the horn on the ground and use it",
	["HS_ORIBOS_OR_FLY"] = "Use HS to Oribos or fly there",
	["IGNORE_MOBS_FAST"] = "Can ignore mobs (if you're fast)",
	["IN_BOAT"] = "In Boat",
	["IN_HOUSE"] = "In the house",
	["IN_HUT"] = "In the Hut",
	["IN_NEST_MUSTHROOM"] = "In nest ontop of Musthroom",
	["IN_THE_BASEMENT"] = "In the basement",
	["IN_THE_LAKE"] = "In the Lake",
	["IN_TOWER"] = "In the Tower",
	["IN_WATER"] = "In the water",
	["IN_WATER_PILLAR"] = "In the water by pillar",
	["INFILTRATORS_ARE_ACROSS_CITY_THEY_GLOW_RED_WHEN_YOU_ARE_CLOSE"] = "Infiltrators are across the city, They glow red when you are close.",
	["INSIDE"] = "Inside",
	["INSIDE_BANK"] = "Inside the bank",
	["INSIDE_BUILDING"] = "Inside the building",
	["INSIDE_CAVE"] = "Inside the cave",
	["INSIDE_HEARTHFIRE_TAVERN"] = "Inside Hearthfire Tavern",
	["INSIDE_HOUSE"] = "Inside the house",
	["INSIDE_INN"] = "Inside the Inn",
	["INSIDE_MOUNTAIN"] = "Inside the mountain",
	["INSIDE_SHACK"] = "Inside the shack",
	["INSIDE_TENT"] = "Inside the tent",
	["INSIDE_TOWER"] = "Inside the tower",
	["INSIDE_TUNNEL"] = "Inside the tunnel",
	["INTO_MINE"] = "Into the Mine",
	["INVESTIGATE_CHARRED_FORECLAW"] = "Investigate Charred Foreclaw",
	["INVESTIGATE_CRACKED_RIB"] = "Investigate Cracked Rib",
	["INVESTIGATE_SEVERED_SPINE"] = "Investigate Severed Spine",
	["ITEM_DROPS_FROM_BARON_ASHFLOW_AT_TOP_OF_THE_LAVA_FLOW"] = "Item drops from Baron Ashflow at the top of the lava flow",
	["ITEM_DROPS_FROM_BARON_CRUSTCORE_AT_BACK_OF_THE_CAVE"] = "Item drops from Baron Crustcore at the back of the cave",
	["ITEM_SOLD"] = "Items were sold for",
	["JOLS_COMMANDS"] = "Follow Jol's commands",
	["JOURNEYMAN_RIDING"] = "Journeyman Riding",
	["JUMP"] = "Jump",
	["JUMP_DOWN_USE_GOBLIN_GLIDER"] = "Jump down; use Goblin Glider",
	["JUMP_OFF"] = "Jump off",
	["JUMP_OFF_BRIDGE"] = "Jump off bridge",
	["JUMP_OFF_CLIFF_EGG_IS_DOWN_BELOW"] = "Jump off cliff; egg is down below",
	["KAJAMITE"] = "Kaja’mite Chunks 10% dmg buff",
	["KALIRI_EGG"] = "Kaliri Egg",
	["KEEP_TRICKSTER_TARGETED"] = "Keep Trickster targeted for auto-Emotes (English Client Only)",
	["KEYBINDS"] = "Set Keybind",
	["KILL_BIG_DRAGON"] = "Kill the big dragon",
	["KILL_BLOOD_ELVES"] = "Kill Blood elves",
	["KILL_BLOODFANG_STALKER"] = "Killing a Bloodfang Stalker spawns a quest",
	["KILL_BRISTLELIMBS"] = "Kill Bristlelimbs to summon Chief",
	["KILL_CATHAN_AND_USE_ZORIGS_TOTEM_OF_RESPITE_ON_THEIR_CORPSE"] = "Kill Cathan and use Zorig's Totem of Respite on their corpse",
	["KILL_CENTAURS"] = "Kill centaurs untill Krom'zar appears",
	["KILL_DARK_IRONS"] = "Kill Dark irons untill boss spawns",
	["KILL_DIHAR_AND_USE_ZORIGS_TOTEM_OF_RESPITE_ON_THEIR_CORPSE"] = "Kill Dihar and use Zorig's Totem of Respite on their corpse",
	["KILL_DRAGONHUNTER_IGORDAN_BONUS_OBJECTIVE"] = "Kill Dragonhunter Igordan; Bonus Objective",
	["KILL_ELEMENTAL"] = "Kill elemental",
	["KILL_ELEMENTALS"] = "Kill elementals",
	["KILL_ELEMENTALS_AND_LOOT_COAL"] = "Kill Elementals and loot Coal",
	["KILL_EXPLOSIVE_HATRED"] = "Kill Explosive Hatreds to disable shield",
	["KILL_FIRAVA_REKINDLER_RESPAWNS_WITHIN_A_MINUTE_WANDERS_AROUND_THE_AREA"] = "Kill Firava the Rekindler, respawns within a minute, wanders around the area",
	["KILL_FLEETFOOT"] = "Kill Fleetfoot and loot his tail",
	["KILL_GANMAT_AND_USE_ZORIGS_TOTEM_OF_RESPITE_ON_THEIR_CORPSE"] = "Kill Ganmat and use Zorig's Totem of Respite on their corpse",
	["KILL_GORGER_AND_LOOT_HORNS"] = "Kill Gorger and loot the horns",
	["KILL_MODAK_FLAMESPIT"] = "Kill Modak Flamespit",
	["KILL_NAGA"] = "Kill naga",
	["KILL_NPCS_FASTER_YSERA_DIES"] = "The faster you kill the NPCs the faster Ysera dies",
	["KILL_OLPHIS_MOLTEN"] = "Kill Olphis the Molten",
	["KILL_PAINMASTER"] = "Kill Painmaster on the wagon",
	["KILL_PIERCER_GIGRA"] = "Kill Piercer Gigra",
	["KILL_SAPPER"] = "Kill Sapper",
	["KILL_SATYR_FLESH"] = "Kill Satyrs for Satyr Flesh",
	["KILL_SNOLL_ICEBREAKER"] = "Kill Snoll the Icebreaker",
	["KILL_SPIRITS_AND_USE_THE_TOTEM_ON_THE_SPIRITS"] = "Kill the spirits and use the totem on the spirits",
	["KILL_WINTERFALL_MOBS"] = "Kill Winterfall mobs",
	["KILL_ZAPHIL_AND_USE_ZORIGS_TOTEM_OF_RESPITE_ON_THEIR_CORPSE"] = "Kill Zaphil and use Zorig's Totem of Respite on their corpse",
	["KIRIN_TOR_MAGES_CAN_BE_SEEN_ON_MINIMAP"] = "Kirin Tor Mages can be seen on minimap.",
	["KITE_GROMLINGS"] = "Click and kite Gromlings to Wildseed",
	["LAND_ON_ROCK"] = "Land on the rock",
	["LAVA_BEACONS_CAN_BE_SEEN_ON_MINIMAP"] = "Lava Beacons can be seen on minimap.",
	["LAVA_ORBS_CAN_BE_SEEN_ON_MINIMAP"] = "Lava Orbs can be seen on minimap",
	["LAVA_ORBS_CAN_BE_SEEN_ON_MINIMAP_KILL_AND_LOOT_RUVIN_STONEGRINDER"] = "Lava Orbs can be seen on minimap. Kill and loot Ruvin Stonegrinder.",
	["LEAN_TAKE_THE_SKIES_SKILL"] = "Lean the 'Take to the Skies' skill",
	["LEAVE_GARRISON_SOUTH"] = "Leave Garrison Go South - chest on wall",
	["LEAVE_PORTAL"] = "Leave the Portal",
	["LEAVE_TUTORIAL"] = "Leave Tutorial",
	["LEFT_STAIRS"] = "Left of stairs",
	["LISTEN"] = "Listen",
	["LOA_INFO_1"] = "Gonk: 40% move speed and half fall damage for 30sec",
	["LOA_INFO_2"] = "Pa'Ku: Travel Zuldazar - 6 differents places ",
	["LOAD"] = "Load",
	["LOADED"] = "Loaded",
	["LOCK_ARROW_WINDOW"] = "Lock Arrow Window",
	["LOCK_QLIST_WINDOW"] = "Lock the QuestList Window",
	["LOOT_BAG"] = "Loot the bag",
	["LOOT_BELONGINGS"] = "Loot Belongings",
	["LOOT_CAMP_PROVISIONS_MARKED_ON_YOUR_MINIMAP_FERALBLOOM_PODS_ARE_AROUND_THE_CAMP"] = "Loot the Camp Provisions, marked on your minimap. Feralbloom Pods are around the camp.",
	["LOOT_COCONUT"] = "Loot Coconut",
	["LOOT_DAGGER"] = "Loot Dagger",
	["LOOT_FEMUR"] = "Loot femur underneath big rock",
	["LOOT_HARPYS_HORN"] = "Loot Horn from Harpys",
	["LOOT_JAR"] = "Loot Jar",
	["LOOT_LANTERN"] = "Loot lantern",
	["LOOT_MELONFRUIT"] = "Loot Melonfruit",
	["LOOT_MURLOC_START_A_QUEST"] = "Loot the murloc to start a quest.",
	["LOOT_OATHSTONE"] = "Loot the Oathstone",
	["LOOT_PAMELA_HEAD"] = "Loot: Pamela's Doll's Head Left and Right Side and combine them",
	["LOOT_RETREAT_ORDERS_FROM_SHELA_WINDBINDER"] = "Loot Retreat Orders from Shela the Windbinder",
	["LOOT_SATYR_SABER"] = "Loot weaponracks for Satyr Sabers",
	["LOOT_SKULLS_AND_SUMMON"] = "Loot skulls in room and summon",
	["LOOT_SPEAR"] = "Loot the spear",
	["LOOT_STAFF"] = "Loot the Staff",
	["LOOT_SWEETSUCKLE_BLOOMS_AROUND_HOUSE_AND_COMBINE_5_OF_THEM"] = "Loot Sweetsuckle Blooms around the house and combine 5 of them.",
	["LOOT_TUSKS"] = "Loot the Tusks",
	["LOOT_WAND"] = "Loot the Wand",
	["LOOT_WILDFIRE_BOTTLE"] = "Loot Bottle of Wildfire from table",
	["LOOTED_FROM_A_SLAIN_MOB"] = "Looted from a slain mob",
	["MAP_WALL"] = "From the Map on the wall",
	["MASTER_RIDING"] = "Master Riding",
	["MAXOMILLIAN_PRAY"] = "Let Maxomillian pray to Furys",
	["MEET_SENDRAX_BY_RITUAL_SITE_SPEAK_HER_WHEN_READY"] = "Meet Sendrax by the ritual site, speak to her when ready",
	["MINIMAP_BLOB_ALPHA"] = "Set Minimap blobs alpha",
	["MINING_PICK"] = "Mining Pick",
	["MISSING_Q"] = "Missing quest(s)",
	["MOTES_CAN_BE_SEEN_ON_MINIMAP"] = "Motes can be seen on minimap",
	["MOUNT_HORSE_SCARE_SPIDER"] = "Mount a Horse and scare Spiders",
	["MOUNT_ON_YOUR_DRAGON"] = "Mount on your dragon",
	["MOUNTING_DESPAWNS_SWARMER"] = "Mounting de-spawns the swarmer",
	["MOVE_EDGE_POINT_YOUR_CAMERA_AT_FIRST_RING_AND_WALK_OFF_EDGE_DO_NOT_JUMP"] = "Move to edge, point your camera at first ring and walk off the edge, do not jump",
	["MOVE_ZONE"] = "Right-click or drag zone to move routes",
	["NAME"] = "Name",
	["NEDD_BUFF_DISARM_TRAP"] = "Needed buffs for Disarming Traps",
	["NEED_CHECK_FPS"] = "Need to check FPs",
	["NEED_HELP"] = "Need help ?",
	["NET_PILLAGER"] = "Net a Pillager and drag it to camp",
	["NO_WAYPOINTS_GARRISON"] = "No waypoints are available while you complete the quest(s)",
	["NORTH_OUTPOST"] = "North of your Outpost",
	["NOT_IN_CHROMIE_TIMELINE"] = "You are not in Chromie Time!",
	["NOT_SKIP_VIDEO"] = "Don't skip video",
	["NOT_YET"] = "Not Yet!",
	["NPC_DROP_Q"] = "If the NPC's aren't talking to each other immediately, then abandon the quest and pick it up again",
	["NPC_TOP_TOWER"] = "NPC is ontop of the tower",
	["NPC_ZENKIKI"] = "Click on the npc (Zen'Kiki) so he pulls Hawks",
	["ON_BUILDING"] = "On the Building",
	["ON_CHAINED_OUTCAST"] = "on the chained outcast",
	["ON_CLIFF"] = "Ontop Cliff",
	["ON_PILLAR"] = "On the pillar",
	["ON_ROCK"] = "On the rock",
	["ON_TOP_OF_A_ROCK"] = "On top of a rock",
	["ON_TOP_OF_BOX"] = "On top of the box",
	["ON_TOP_OF_BUILDING"] = "On top of building",
	["ON_TOP_OF_TOWER"] = "On top of the tower",
	["ONLY_ONE_WAIT_NPC"] = "Only one can do the quest at a time so you might have to wait for npc to respawn",
	["OPEN_BAG"] = "Open Bag",
	["OPTIONAL"] = "Optional",
	["OUT_CAVE"] = "Exit cave",
	["OUT_WEST_GATE"] = "Outside West gate",
	["PATROLS_AROUND_AREA"] = "Patrols around the area",
	["PHOTOGRAPH_DRAGON"] = "Photograph the dragon",
	["PICK_ARSENAL"] = "Pick Arsenal",
	["PICK_ROUTE"] = "Pick Route",
	["PICK_UP_BOW"] = "Pick up bow",
	["PICK_UP_FROM_SENDRAX_WHO_SHOULD_BE_STANDING_NEXT_YOU"] = "Pick up from Sendrax, who should be standing next to you",
	["PICK_UP_LETTER"] = "Pick up letter from mail",
	["PICK_UP_Q"] = "Pick up the quest(s)",
	["PICK_UP_SPEAR"] = "Pick up spear",
	["PICK_UP_TORCH"] = "Pick up torch",
	["PICK_ZONE"] = "Pick Zone",
	["PILLAR_INSIDE_CAVE"] = "Pillar inside the cave",
	["PINEWOOD_POST_HS"] = "Pinewood Post HS",
	["PLACE_FRAGMENTS"] = "Place the Fragments",
	["PLACE_LIGHTWELLS"] = "Place Lightwells around the corpsebeasts",
	["PLACE_MEAT"] = "Open quest bag place meat use pheromones",
	["PLACE_TOTEMS"] = "Place totems",
	["PLACE_TOTEMS_ORDER_DOES_NOT_MATTER"] = "Place the totems, order does not matter",
	["PORTAL_WILL_APPEAR"] = "Portal will Appear",
	["POSTER_ON_POLE"] = "Poster on the pole",
	["PRESS_1"] = "Press 1",
	["PRESS_2"] = "Press 2",
	["PRESS_3"] = "Press 3",
	["PRIMAL_PROTODRAKE"] = "Primal Proto-Drake",
	["PULL_OUT_SPEAR"] = "Pull out the spear",
	["PUS_PET_ABILITY"] = "Use Pet ability (Call to Arms) to Enlist Troops",
	["PUSH_BOAT"] = "Push the boat",
	["Q_ACCEPTED"] = "Quest(s) accepted",
	["Q_BTN_SCALE"] = "Quest Buttons Scale",
	["Q_DROP"] = "Quest Drop",
	["Q_OPTIONS"] = "Quest Options",
	["Q_PART"] = "Do part of the quest",
	["Q_REMOVED"] = "Quest removed",
	["Q_TEXT"] = "Quests Text",
	["QLIST_OUT_SCREEN"] = "The QuestList is off the screen! Resetting its position...",
	["QLIST_SCALE"] = "QuestList Scale",
	["QORDERLIST_SCALE"] = "Quest Order List Scale",
	["RARE__YOU_SHOULD_ONLY_HAVE_BRING_HIM_TO_40_HP_TO_DEFEAT_HIM"] = "Rare : You should only have to bring him to 40% hp to defeat him",
	["READ_BATTLE_PLANS_INSIDE_THE_HUT"] = "Read the Battle Plans inside the hut",
	["REDUCED_DAMAGE_INFO_1"] = "Wait until his stack of buffs are gone",
	["REPAIR_EQUIPEMENT"] = "Equipment has been repaired for",
	["RESCUE_AZURE_EGG"] = "Rescue Azure egg",
	["RESCUE_BRONZE_EGG"] = "Rescue Bronze egg",
	["RESCUE_EMERALD_EGG"] = "Rescue Emerald egg",
	["RESCUE_RUBY_EGG"] = "Rescue Ruby egg",
	["RESET"] = "reset",
	["RESET_ARROW"] = "Reset Arrow",
	["RESET_QORDERLIST"] = "Reset QuestOrderList",
	["RESET_ZONE"] = "Resetting Zone",
	["RIDE_BOOK"] = "Ride the book",
	["RIDE_MAJORDOMO_SELISTRA_RUBY_LIFESHRINE"] = "Ride Majordomo Selistra to Ruby Lifeshrine",
	["RIDE_NISHA"] = "Ride Nisha",
	["RIDE_TOME_OF_SPELLFLINGING"] = "Ride the Tome of Spellflinging",
	["RIDE_WRATHION_THRONE"] = "Ride Wrathion to the throne",
	["RIDE_WRATHION_TOP"] = "Ride Wrathion to the top",
	["RIDE_YOUR_DRAGON_NEXT_ARBALEST"] = "Ride your dragon to next Arbalest",
	["RIDE_YOUR_DRAGON_OR_WRATHION_NEXT_ARBALEST"] = "Ride your dragon or Wrathion to next Arbalest",
	["RING_IN_WATER"] = "Small ring In the water",
	["ROLLBACK"] = "Rollback Quest Step",
	["ROUTE_COMPLETED"] = "Route Completed",
	["ROUTE_NOT_FOUND"] = "Route Not found for",
	["RUN_FOREST_RUN"] = "DON’T WAIT RUN THIS WAY!!!",
	["RUN_UP_HILL"] = "Run up the hill",
	["RUN_WAYPOINT"] = "Run to the waypoint",
	["SAIL_TO"] = "Sail to",
	["SALVAGE_FLOATING_DEBRIS"] = "Salvage floating debris",
	["SAVAGE_FIGHT_CLUB"] = "Chose The Savage Fight Club",
	["SCARE_SPIDER_INTO_LUMBERMILL"] = "Scare Spiders into the Lumbermill",
	["SCENARIO"] = "Do the scenario",
	["SCOUTING_MAP"] = "From the scouting map",
	["SEARCH_BAG"] = "Search the bag",
	["SEARCH_CENTAUR_TENTS"] = "Search the Centaur tents",
	["SECOND_CLUE"] = "Second clue",
	["SELECT_OPTION_1_THEN_4_THEN_2_THEN_3"] = "Select Option 1, then 4, then 2, then 3",
	["SET_HEARTHSTONE"] = "Set Hearthstone",
	["SETTINGS"] = "Settings",
	["SHACKLE_HUT"] = "Click the shackle in the Hut",
	["SHAMANS_CAN_BE_SEEN_ON_YOUR_MINIMAP"] = "Shamans can be seen on your minimap",
	["SHARDS_CAN_BE_SEEN_ON_MINIMAP"] = "Shards can be seen on minimap",
	["SHIMMERDUST_BUFF"] = "Shimmerdust Pile buff",
	["SHOOT_DRAGONS"] = "Shoot the dragons",
	["SHOOT_DRAGONS_USING_ARCANE_BLASTER"] = "Shoot the dragons using Arcane Blaster",
	["SHOW_ALEXSTRASZA_EGG"] = "Show Alexstrasza the egg",
	["SHOW_ARROW"] = "Show Arrow",
	["SHOW_BLOBS_ON_MAP"] = "Show Green blobs on map",
	["SHOW_BLOBS_ON_MINIMAP"] = "Show Green blobs on minimap",
	["SHOW_GROUP_PROGRESS"] = "Show Group Progress",
	["SHOW_QLIST"] = "Show QuestList",
	["SHOW_QORDERLIST"] = "Show Quest Order List",
	["SHOW_RIDING"] = "Show mount skill learning messages",
	["SHOW_STEPS_MAP"] = "Show 10 steps on map",
	["SKIP"] = "Skipping Quest Step",
	["SKIP_BUTTON"] = "Skip",
	["SKIP_WAYPOINT"] = "Skip to the waypoint",
	["SKIP_WAYPOINT_IF_YOU_GLIDE"] = "If you can glide, skip the waypoint and glide to the to quest(s) mobs",
	["SKIPCAMP"] = "Skipping Camp Step",
	["SKIPPED_CUTSCENE"] = "Skip cutscene",
	["SLAY_TWISTED_ANCIENT"] = "Slay the Twisted Ancient",
	["SOULWEB_TRAP"] = "Suspiciously Untouched Soulweb",
	["SPEAK_CELORMU"] = "Speak to Celormu",
	["SPEAK_DEATHLY_USHER"] = "Speak to Deathly Usher",
	["SPEAK_KALECGOS"] = "Speak to Kalecgos",
	["SPEAK_MOTHER_ELION"] = "Speak to Mother Elion",
	["SPEAK_OHN_SESHTENG"] = "Speak to Ohn Seshteng",
	["SPEAK_SENDRAX_SEND_SIGNAL_FLARES"] = "Speak to Sendrax; send signal flares",
	["SPEAK_WITH_CELORMU_START_RACE"] = "Speak with Celormu to start the race",
	["SPEAK_WITH_LITHRAGOSA"] = "Speak with Lithragosa",
	["SPEAK_WITH_SABELLIAN"] = "Speak with Sabellian",
	["SPEAK_ZAHKRANA"] = "Speak to Zahkrana",
	["SPEAR_DOWN_THE_UFO"] = "He is flying use spear to pull him",
	["SPEEDRUN"] = "Speed Run",
	["SUGGESTED_PLAYERS"] = "Suggested Players",
	["SWITCH_TO_CHROMIE"] = "Go to Chromie and change your timeline",
	["TAKE_BOAT_TO_SEEKERS_OUTPOST"] = "Take Boat to Seeker's Outpost",
	["TAKE_DRAGON"] = "Take the dragon",
	["TAKE_FEATHER"] = "Take the Feather",
	["TAKE_HORN"] = "Take the Horn",
	["TAKE_OUT_3_BRACKENHIDE_PUTRIFIERS"] = "Take out 3 Brackenhide Putrifiers",
	["TAKE_SOIL"] = "Take the Soil",
	["TAKE_SPEAR"] = "Take the Spear",
	["TALK_ALEXSTRASZA"] = "Talk to Alexstrasza",
	["TALK_ARCHIVIST_EDRESS"] = "Talk to Archivist Edress",
	["TALK_ARCHIVIST_EDRESS_TWICE"] = "Talk to Archivist Edress twice",
	["TALK_BASKILIAN"] = "Talk to Baskilian",
	["TALK_DRAGON"] = "Talk to the dragon",
	["TALK_DREAD_RIDER_CULLEN"] = "Talk to Dread-Rider Cullen",
	["TALK_EFFIGY"] = "Talk to Effigy",
	["TALK_ENSIGN_WARD"] = "Talk to Ensign Ward",
	["TALK_ERUL"] = "Talk to Erul Dawnbrook",
	["TALK_FORGEMASTER_BAZENTUS"] = "Talk to Forgemaster Bazentus",
	["TALK_GRAND_AMIRAL_JES_TERETH"] = "Talk to Grand Admiral Jes-Tereth",
	["TALK_HALANNIA"] = "Talk to Halannia",
	["TALK_JHASH"] = "Talk to Jhash for a ride",
	["TALK_JORNUN"] = "Talk to Jornun to Ride",
	["TALK_KING_RASTAKHAN"] = "Talk to King Rastakhan",
	["TALK_LEFT_AND_RIGHT"] = "Talk to Left and Right",
	["TALK_NISHA"] = "Talk to Nisha",
	["TALK_NPC"] = "Talk to the NPC",
	["TALK_ORKUS"] = "Talk to Orkus after RP and then loot Plans",
	["TALK_ROKHAN_RIDE_DOWN"] = "Speak to Rokhan for a ride down",
	["TALK_ROKHAN_RIDE_ZULJAN"] = "Speak to Rokhan for a ride to Zul'jan",
	["TALK_SASSY"] = "Talk to Sassy Hardwrench for a ride",
	["TALK_SENDRAX"] = "Talk to Sendrax",
	["TALK_SENTINEL_AVANA"] = "Talk to Sentinel Avana",
	["TALK_STEWARD"] = "Talk to Steward",
	["TALK_SWELLTHRASHER"] = "Talk to Swellthrasher",
	["TALK_TALONSTALKER_KAVIA"] = "Talk to Talonstalker Kavia",
	["TALK_THELARON"] = "Talk to Thelaron",
	["TALK_TO"] = "Talk to",
	["TALK_TO_NPC_TO_RIDE_BOAT"] = "Talk to NPC to ride boat",
	["TALK_WITH_ARCHIVIST_EDRES_WAIT_FOR_HER_DIALOG_COMPLETE"] = "Talk with Archivist Edres, wait for her dialog to complete",
	["TALK_WITH_BLACKTALON_ASSASSIN_START_COMBAT"] = "Talk with Blacktalon Assassin to start the combat.",
	["TALK_WITH_FAO_THE_RELENTLESS"] = "Talk with Fao The Relentless",
	["TALK_WITH_FAO_THE_RELENTLESS_THEN_CLICK_BATTLE_PLANS_INSIDE_HUT"] = "Talk with Fao The Relentless, then click Battle Plans inside the hut",
	["TALK_WITH_FORGEMASTER_BAZENTUS"] = "Talk with Forgemaster Bazentus",
	["TALK_WITH_TALONSTALKER_KAVLA_THEN_USE_TELESCOPE"] = "Talk with Talonstalker Kavla, then use the Telescope",
	["TARGET_AZURE_DEFENDERS_USE_WAND"] = "Target Azure Defenders, use wand",
	["TARGET_AZURE_DEFENDERSUSE_WAND"] = "Target Azure Defendersnuse wand",
	["TAVERN"] = "Tavern",
	["TELEPORT_UP_AND_JUMP"] = "Teleport up and jump in the ring",
	["THE_CHOICES_DO_NOT_MATTER"] = "The choices do not matter",
	["THIRD_CLUE"] = "Third clue",
	["TO_CAMP_ANTODINAS"] = "To Camp Antodinas",
	["TO_CONJURED_BISCUIT_INN"] = "To Conjured Biscuit Inn",
	["TO_GAZLOWE"] = "To Gazlowe",
	["TO_PINEWOOD_POST"] = "To Pinewood Post",
	["TO_ROKHAN_GAZLOWE"] = "Turn in to Rokhan and Gazlowe",
	["TO_WARMASTER_ZOG"] = "To Warmaster Zog inside the Great Hall",
	["TOP_BIG_MUSHROOM"] = "Ontop the big mushroom",
	["TOP_BOAT"] = "He is Up top of the boat",
	["TOP_OF_MOUNTAIN"] = "Ontop of the mountain",
	["TOP_TOWER_OVER_EDGE"] = "Ontop of the tower over the edge",
	["TOTEM_DAMAGE_BUFF"] = "Stand in totem for dmg buff",
	["TRAIN_APPRENTICE_RIDING"] = "Train Apprentice Riding",
	["TRAIN_RIDING"] = "Train Riding",
	["TRAVEL_TO"] = "Travel to",
	["TREASURE_CAVE"] = "Treasure in cave",
	["TREASURE_NORTH_SHIP"] = "Treasure is North of ship",
	["TREASURE_TOP_TOWER"] = "Treasure is ontop of the tower",
	["TURN_IN_INITIATE_ZORIG_WHO_SHOULD_BE_RIGHT_BESIDE_YOU"] = "Turn in to Initiate Zorig who should be right beside you",
	["TURN_IN_Q"] = "Turn-in the quest(s)",
	["TURN_IN_SENDRAX_WHO_SHOULD_BE_STANDING_NEXT_TO_YOU"] = "Turn in to Sendrax, who should be standing next to you",
	["TURN_ON_WARMODE"] = "Turn on War Mode for extra experience!",
	["TWO_DOWNSTAIRS_ONE_UPSTAIRS"] = "Two downstairs, one upstairs",
	["UNDERNEATH"] = "Underneath",
	["UNDERNEATH_BRIDGE"] = "Underneath bridge",
	["UNSTABLE_ARCANE_CAN_BE_SEEN_ON_MINIMAP"] = "Unstable Arcane can be seen on minimap",
	["UNTOUCHED_BASKET"] = "Suspiciously Untouched Basket",
	["UP_CRANE"] = "Up in the crane",
	["UP_ELEVATOR"] = "Go up the Elevator",
	["UP_RAMP"] = "Up the ramp",
	["UP_TOWER"] = "Up the Tower",
	["UP_TREE"] = "Up in the tree",
	["UPDATE_ARROW"] = "Update Arrow every",
	["UPSTAIRS"] = "Upstairs",
	["UPSTAIRS_AND_JUMP"] = "Go upstairs and jump over on the ledge",
	["UPSTAIRS_BED"] = "Upstairs in bed",
	["UPTOP"] = "Uptop",
	["USE_ARCANE_BLASTER_KILL_DRAGONS"] = "Use Arcane Blaster to kill dragons",
	["USE_ASTRANAAR_THROWER"] = "Use Astranaar Thrower",
	["USE_BAG"] = "Use the Bag",
	["USE_BANNER_ON_CORPSES"] = "Use Banner on Corpses",
	["USE_BARRELS"] = "Use the barrels to weaken Mucksquint",
	["USE_BOAT"] = "Use Boat",
	["USE_BRANCH_ON_GORMS"] = "Use the branch on large hostile Gorms before you kill them",
	["USE_CANNON"] = "Use cannon to get back",
	["USE_CART"] = "Use Cart",
	["USE_CHAIR"] = "Use Chair",
	["USE_CLAW"] = "Use the Claw",
	["USE_COVENANT_ABILITY"] = "Use your Soulshape covenant ability",
	["USE_CRYSTAL_PYLON"] = "Use the Crystal Pylon",
	["USE_DALARAN_HEARTHSTONE"] = "Use Dalaran Hearthstone",
	["USE_DISGUISE"] = "Use Disguise",
	["USE_ELEVATOR"] = "Use the elevator",
	["USE_FLIGHTPATH"] = "Use Flightpath",
	["USE_GARRISON_HEARTHSTONE"] = "Use Garrison Hearthstone",
	["USE_GATE"] = "Use Gate",
	["USE_GATE_MALDRAXXUS"] = "Use Gate to Maldraxxus",
	["USE_GATEWAY"] = "Use the gateway",
	["USE_GLAIVE_THROWER"] = "Use Glaive Thrower",
	["USE_HEARTHSTONE"] = "Use Hearthstone",
	["USE_ITEM"] = "Use Item",
	["USE_ITEM_FOR_DAMAGE_BOOST"] = "Use the item for damage boost",
	["USE_ITEM_ON_ARGALI"] = "Use Item on Argali",
	["USE_JAVELINS_TO_SPEED"] = "Loot and use Javelins to speed up killing",
	["USE_MOUNT"] = "Use Mount",
	["USE_ORB_CANYON_ETTIN"] = "Use Orb on a Canyon Ettin then save Oslow",
	["USE_ORGRIMMAR_PORTAL"] = "Use Orgrimmar Portal",
	["USE_PHASEBLOOD_POTION"] = "Use Phaseblood Potion on Mark",
	["USE_PORTAL"] = "Use the portal",
	["USE_PORTAL_TO"] = "Use Portal to",
	["USE_QITEM_DUNGEON"] = "Use quest item inside dungeon",
	["USE_REVEALING_DRAGONS_EYE"] = "Use Revealing Dragon's Eye",
	["USE_SOULBREAKER"] = "Use Soulbreaker Traps for extra dmg",
	["USE_SOULSHAPE"] = "Use Soulshape",
	["USE_SURGE_FORWARD_PASS_FIRST_FOUR_RINGS_USE_ASCEND_TO_REACH_RING_FIVE_AND_ONCE_MORE_JUST_AFTER"] = "Use Surge Forward to pass first four rings; use ascend to reach ring five and once more just after",
	["USE_TAVERN_HEARTHSTONE"] = "Use Tavern Hearthstone to camp",
	["USE_TELEPORT"] = "Use teleport",
	["USE_TELEPORT_THE_TOP"] = "Use the teleport to the top",
	["USE_TELESCOPE"] = "Use the Telescope",
	["USE_TORNADO_IN_FRONT_OF_MERITHRA_FOR_BOOST"] = "Use the tornado in front of Merithra for boost",
	["USE_WHISTLE_ON_RAMS"] = "use /whistle on rams",
	["VELOMIRS_UNITS_ARE_MARKED_ON_MINIMAP"] = "Velomir's Units are marked on the minimap",
	["WAIT_FOR_DERVISHIAN_BE_IN_FRONT_OF_YOU_TO_TAKE_PICTURE"] = "Wait for Dervishian to be in front of you to take picture",
	["WAIT_FOR_NPC"] = "Wait for NPC to get in place",
	["WAIT_FOR_NPC_END_RP"] = "Wait for NPC to end RP",
	["WAIT_FOR_NPC_SPAWN"] = "Wait for NPC to spawn",
	["WAIT_FOR_RP_END"] = "Wait for RP to end",
	["WAIT_FOR_SHIP"] = "Wait for the ship",
	["WAIT_FOR_ZEPPELIN"] = "Wait for the zeppelin",
	["WALK_BLADEGUARD_KAJA"] = "Walk with Bladeguard Kaja",
	["WALK_OVER_SPRING"] = "Walk over the spring",
	["WALK_TRAMPOLINE"] = "Walk to the trampoline",
	["WATERFALLS_CAN_BE_SEEN_ON_YOUR_MINIMAP"] = "Waterfalls can be seen on your minimap",
	["WAYPOINT"] = "Waypoint",
	["WEAPON_PICK_DOESNT_MATTER"] = "Doesn't matter what weapon you pick",
	["WELCOME"] = "Welcome to Azeroth Pilot Reloaded!",
	["WELCOME_DEV"] = "Developers",
	["WELCOME_GLIDER"] = "Note, some routes may require a Goblin Glider Kit.",
	["WELCOME_SUP"] = "Support",
	["WELCOME_ZYRR"] = "Special thanks to Zyrrael for making the original AAP.",
	["WHELPS_ARE_CONVENIENTLY_MARKED_ON_YOUR_MAP"] = "Whelps are conveniently marked on your map",
	["YARDS"] = "yards",
	["YES_TO_EXIT_TUTORIAL"] = "Yes To Exit Tutorial",
	["YOU_CAN_LEARN"] = "You can now learn",
	["YOU_DO_NOT_HAVE_RIDE_WITH_CARAVAN_OR_WAIT_FOR_IT"] = "You do not have to ride with the caravan or wait for it.",
	["YOU_SHOULD_ONLY_HAVE_BRING_HIM_TO_40_HP_TO_DEFEAT_HIM"] = "You should only have to bring him to 40% hp to defeat him",
	["ZONE"] = "Zone",
	["ZONE_DONE"] = "Zone Done"
}