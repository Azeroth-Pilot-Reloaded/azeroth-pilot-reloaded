...
local QSpecial57710 = 0
local Q2Special57710 = 0
APR.GossipOpen = 0
APR.BookingList = {}
APR.HBDP = HBDP
APR.HBD = HBD
...
function APR:MoveIcons()
	...
	elseif (CurStep and APR.ActiveMap and APR.QStepList and APR.QStepList[APR.ActiveMap] and APR.QStepList[APR.ActiveMap][CurStep]) then
	...
	if (steps["CRange"]) then
		local CLi
		local totalCR = 1
		if (APR.QStepList[APR.ActiveMap][CurStep+1] and APR.QStepList[APR.ActiveMap][CurStep+1]["CRange"]) then
			totalCR = 3
		end
		...
		local px, py = GetPlayerMapPos(C_Map.GetBestMapForUnit('player'), APR.QStepList[APR.ActiveMap][CurStep]["TT"]["y"],APR.QStepList[APR.ActiveMap][CurStep]["TT"]["x"])
		local CLi, CLi2
		if (not APR.QStepList[APR.ActiveMap][CurStep+1]) then
			...
			local ix, iy = GetPlayerMapPos(C_Map.GetBestMapForUnit('player'), APR.QStepList[APR.ActiveMap][CurStep+1]["TT"]["y"],APR.QStepList[APR.ActiveMap][CurStep+1]["TT"]["x"])
			for CLi = 1, 20 do
				local px2, py2
				px2 = px - ix
				py2 = py - iy
		...
		if (totalCR == 3) then
			...
			local px, py = GetPlayerMapPos(C_Map.GetBestMapForUnit('player'), APR.QStepList[APR.ActiveMap][CurStep+1]["TT"]["y"],APR.QStepList[APR.ActiveMap][CurStep+1]["TT"]["x"])
			local CLi, CLi2
			local ix, iy = GetPlayerMapPos(C_Map.GetBestMapForUnit('player'), APR.QStepList[APR.ActiveMap][CurStep+2]["TT"]["y"],APR.QStepList[APR.ActiveMap][CurStep+2]["TT"]["x"])
			for CLi = 1, 20 do
				local px2, py2
				px2 = px - ix
				py2 = py - iy
...
function APR:MoveMapIcons()
	...
	if (steps["CRange"]) then
		...
		local px, py = GetPlayerMapPos(SetMapIDs, APR.QStepList[APR.ActiveMap][CurStep]["TT"]["y"],APR.QStepList[APR.ActiveMap][CurStep]["TT"]["x"])
		local CLi, CLi2
		if (not APR.QStepList[APR.ActiveMap][CurStep+1]) then
			for CLi = 1, 20 do
				APR.HBDP:RemoveWorldMapIcon("APRMap", APR["MapIcons"][CLi])
			...
			local ix, iy = GetPlayerMapPos(SetMapIDs, APR.QStepList[APR.ActiveMap][CurStep+1]["TT"]["y"],APR.QStepList[APR.ActiveMap][CurStep+1]["TT"]["x"])
			for CLi = 1, 20 do
				local px2, py2
				px2 = px - ix
				py2 = py - iy
		...
		if (totalCR == 3) then
			...
			local px, py = GetPlayerMapPos(SetMapIDs, APR.QStepList[APR.ActiveMap][CurStep+1]["TT"]["y"],APR.QStepList[APR.ActiveMap][CurStep+1]["TT"]["x"])
			local CLi, CLi2
			local ix, iy = GetPlayerMapPos(SetMapIDs, APR.QStepList[APR.ActiveMap][CurStep+2]["TT"]["y"],APR.QStepList[APR.ActiveMap][CurStep+2]["TT"]["x"])
			for CLi = 1, 20 do
				local px2, py2
				px2 = px - ix
				py2 = py - iy
...
local function APR_SettingsButtons()
	...
	for CLi = 1, 3 do
		...
		APR.QList2["BF"..CLi]["APR_Buttonptex"]:SetTexture(itemTexture)
		APR.QList2["BF"..CLi]["APR_Buttonntex"]:SetTexture(itemTexture)
		--APR.QList2["BF"..CLi]["APR_Button"]:SetNormalTexture(itemTexture)
		APR.QList2["BF"..CLi]["APR_Button"]:SetText("")
		local Topz = APR1[APR.Realm][APR.Name]["Settings"]["left"]
		local Topz2 = APR1[APR.Realm][APR.Name]["Settings"]["top"]
		APR.QList20:SetPoint("TOPLEFT", UIParent, "TOPLEFT", Topz, Topz2)
		APR.QList2["BF"..CLi]:SetPoint("BOTTOMLEFT", APR.QList21, "BOTTOMLEFT",0,-((CLi * 38)+CLi))
		APR.QList2["BF"..CLi]:Show()
	end
end
function APR.ChkBreadcrums(qids)
	if (qids and APR.Breadcrums and APR.Breadcrums[qids]) then
		for APR_index,APR_value in pairs(APR.Breadcrums[qids]) do
			if ((APR.ActiveQs[APR_value] or C_QLog.IsQFlaggedCompleted(APR_value) == true) and (not APR.ActiveQs[qids])) then
...
local function APR_LeaveQ(QIDs)
	C_QLog.SetSelectedQ(QIDs)
	C_QLog.AbandonQ()
end
...
local function APR_QAskPopWanted()
	...
	if (C_QLog.IsQFlaggedCompleted(Qid) == true) then
		APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
		APR.BookingList["PrintQStep"] = 1
		APR.QList.SugQFrame:Hide()
	elseif (steps["QLineSkip"] and APR1[APR.Realm][APR.Name]["QlineSkip"][steps["QLineSkip"]] and APR1[APR.Realm][APR.Name]["QlineSkip"][steps["QLineSkip"]] == 0) then
		APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
		APR.BookingList["PrintQStep"] = 1
	else
		local SugGroupNr = steps["Group"]
		APR.QList.SugQFrameFS1:SetText(L["OPTIONAL"])
		APR.QList.SugQFrameFS2:SetText(L["SUGGESTED_PLAYERS"]..": "..SugGroupNr)
		APR.QList.SugQFrame:Show()
	end
end
function APR.QAskPopWantedAsk(APR_answer)
	...
	if (APR_answer == "yes") then
		APR1[APR.Realm][APR.Name]["WantedQList"][steps["QaskPopup"]] = 1
		APR.QList.SugQFrame:Hide()
		APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
		APR.BookingList["PrintQStep"] = 1
	else
		APR.QList.SugQFrame:Hide()
		APR1[APR.Realm][APR.Name]["WantedQList"][steps["QaskPopup"]] = 0
		APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
		APR.BookingList["PrintQStep"] = 1
	end
end
local function APR_PrintQStep()
	...
	if (IsInInstance()) then
		...
		APR.ZoneQOrder:Hide()
		return
	elseif (APR1[APR.Realm][APR.Name]["Settings"] and APR1[APR.Realm][APR.Name]["Settings"]["ShowQListOrder"] and APR1[APR.Realm][APR.Name]["Settings"]["ShowQListOrder"] == 1) then
		APR.ZoneQOrder:Show()
	end
	...
	if (APR.SettingsOpen == 1) then
		...
		for i=1,3 do
			LineNr = LineNr + 1
			APR.QList.QFrames["FS"..LineNr]:SetText("Test Q number "..i)
			APR.QList.QFrames[LineNr]:Show()
			APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
			local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
			if (APRwidth and APRwidth > 400) then
				APR.QList.QFrames[LineNr]:SetWidth(APRwidth+10)
			else
				APR.QList.QFrames[LineNr]:SetWidth(410)
			end
		end
		return
	end

	if (APR.ActiveMap and APR.QStepList and APR.QStepList[APR.ActiveMap] and APR.ProgressText and APR.ProgressShown == 1) then
		APR.QList.QFrames["MyProgress"]:Show()
		APR.QList.QFrames["MyProgressFS"]:SetText(APR.ProgressText)
	else
		APR.QList.QFrames["MyProgress"]:Hide()
	end
	if (APR.QStepList and APR.QStepList[APR.ActiveMap] and APR.QStepList[APR.ActiveMap][CurStep]) then
		local steps = APR.QStepList[APR.ActiveMap][CurStep]
		local StepP, IdList
		if (APRExtraText and APR.ZoneTransfer == 0) then
			if (APRExtraText.Paths and APRExtraText.Paths[APR.ActiveMap] and APRExtraText.Paths[APR.ActiveMap][CurStep]) then
				LineNr = LineNr + 1
				APR.QList.QFrames["FS"..LineNr]:SetText(APRExtraText.Paths[APR.ActiveMap][CurStep])
				APR.QList.QFrames[LineNr]:Show()
				APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
				local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
				if (APRwidth and APRwidth > 400) then
		...
		if (steps and steps["LoaPick"] and steps["LoaPick"] == 123 and ((APR.ActiveQs[47440] or C_QLog.IsQFlaggedCompleted(47440)) or (APR.ActiveQs[47439] or C_QLog.IsQFlaggedCompleted(47439)))) then
			APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
			APR.BookingList["PrintQStep"] = 1
			return
		elseif (steps["PickedLoa"] and steps["PickedLoa"] == 2 and (APR.ActiveQs[47440] or C_QLog.IsQFlaggedCompleted(47440))) then
			APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
			APR.BookingList["PrintQStep"] = 1
			if (APR1["Debug"]) then
			...
			return
		elseif (steps["PickedLoa"] and steps["PickedLoa"] == 1 and (APR.ActiveQs[47439] or C_QLog.IsQFlaggedCompleted(47439))) then
			APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
			APR.BookingList["PrintQStep"] = 1
			if (APR1["Debug"]) then
		...
		if (C_QLog.IsQFlaggedCompleted(47440) == true) then
			APR1[APR.Realm][APR.Name]["LoaPick"] = 1
		elseif (C_QLog.IsQFlaggedCompleted(47439) == true) then
			APR1[APR.Realm][APR.Name]["LoaPick"] = 2
		end
		if (steps["LeaveQ"]) then
			APR_LeaveQ(steps["LeaveQ"])
		end
		if (steps["LeaveQs"]) then
			for APR_index,APR_value in pairs(steps["LeaveQs"]) do
				APR_LeaveQ(APR_value)
			end
		end
		if (APR1["Debug"]) then
			print(StepP)
		...
		if (steps["ZoneDoneSave"]) then
			local Maps
			if (APR.QStepListListing["Shadowlands"][APR.ActiveMap]) then
				Maps = APR.QStepListListing["Shadowlands"][APR.ActiveMap]
			elseif (APR.QStepListListing["Kalimdor"][APR.ActiveMap]) then
				Maps = APR.QStepListListing["Kalimdor"][APR.ActiveMap]
			elseif (APR.QStepListListing["SpeedRun"][APR.ActiveMap]) then
				Maps = APR.QStepListListing["SpeedRun"][APR.ActiveMap]
			elseif (APR.QStepListListing["EasternKingdom"][APR.ActiveMap]) then
				Maps = APR.QStepListListing["EasternKingdom"][APR.ActiveMap]
			elseif (APR.QStepListListingStartAreas["EasternKingdom"] and APR.QStepListListingStartAreas["EasternKingdom"][APR.ActiveMap]) then
				Maps = APR.QStepListListingStartAreas["EasternKingdom"][APR.ActiveMap]
			elseif (APR.QStepListListingStartAreas["Kalimdor"] and APR.QStepListListingStartAreas["Kalimdor"][APR.ActiveMap]) then
				Maps = APR.QStepListListingStartAreas["Kalimdor"][APR.ActiveMap]
			elseif (APR.QStepListListing["Extra"][APR.ActiveMap]) then
				Maps = APR.QStepListListing["Extra"][APR.ActiveMap]
			elseif (APR_Custom[APR.Name.."-"..APR.Realm] and APR_Custom[APR.Name.."-"..APR.Realm][APR.ActiveMap]) then
				Maps = APR_Custom[APR.Name.."-"..APR.Realm][APR.ActiveMap]
			end
			if (Maps) then
				APR_ZoneComplete[APR.Name.."-"..APR.Realm][Maps] = 1
				for CLi = 1, 19 do
					if (APR.RoutePlan.FG1["Fxz2Custom"..CLi]["FS"]:GetText() == Maps) then
		...
		if (steps["SpecialFlight"] and C_QLog.IsQFlaggedCompleted(steps["SpecialFlight"])) then
			APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
			APR.BookingList["PrintQStep"] = 1
		end
		if (steps["GroupTask"] and APR1[APR.Realm][APR.Name]["WantedQList"][steps["GroupTask"]] and APR1[APR.Realm][APR.Name]["WantedQList"][steps["GroupTask"]] == 0) then
			APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
			APR.BookingList["PrintQStep"] = 1
			return
		end
		...
		if (steps["UseGlider"] and APR.ZoneTransfer == 0) then
			if (APR1[APR.Realm][APR.Name]["Settings"]["ShowQList"] == 1) then
				LineNr = LineNr + 1
				APR.QList.QFrames["FS"..LineNr]:SetText(L["USE_ITEM"]..": "..APR.GliderFunc())
				APR.QList.QFrames[LineNr]:Show()
				APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
				local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
				if (APRwidth and APRwidth > 400) then
		...
		if (steps["Bloodlust"] and APR.ZoneTransfer == 0) then
			if (APR1[APR.Realm][APR.Name]["Settings"]["ShowQList"] == 1) then
				LineNr = LineNr + 1
				APR.QList.QFrames["FS"..LineNr]:SetText(TextWithStars(L["BLOODLUST"]))
				APR.QList.QFrames[LineNr]:Show()
				APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
				local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
				if (APRwidth and APRwidth > 400) then
		...
		if (steps["InVehicle"] and not UnitInVehicle("player") and APR.ZoneTransfer == 0) then
			LineNr = LineNr + 1
			APR.QList.QFrames["FS"..LineNr]:SetText(L["MOUNT_HORSE_SCARE_SPIDER"])
			APR.QList.QFrames[LineNr]:Show()
			APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
			local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
			if (APRwidth and APRwidth > 400) then
		...
		elseif (steps["InVehicle"] and steps["InVehicle"] == 2 and UnitInVehicle("player") and APR.ZoneTransfer == 0) then
			LineNr = LineNr + 1
			APR.QList.QFrames["FS"..LineNr]:SetText(L["SCARE_SPIDER_INTO_LUMBERMILL"])
			APR.QList.QFrames[LineNr]:Show()
			APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
			local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
			if (APRwidth and APRwidth > 400) then
		...
		if (steps["DalaranToOgri"] and APR.ZoneTransfer == 0) then
			if (APR1[APR.Realm][APR.Name]["Settings"]["ShowQList"] == 1) then
				LineNr = LineNr + 1
				APR.QList.QFrames["FS"..LineNr]:SetText(L["GO_TO"].." Orgrimmar")
				APR.QList.QFrames[LineNr]:Show()
				APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
				local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
				if (APRwidth and APRwidth > 400) then
		...
		function DisplayRiding(text)
			if(not APR1[APR.Realm][APR.Name]["hideRidingSkill"] or APR1[APR.Realm][APR.Name]["hideRidingSkill"] == 0) then
				LineNr = LineNr + 1
				APR.QList.QFrames["FS"..LineNr]:SetText(TextWithStars(text))
				--skip waypoint button
				-- TODO : fix display button
				AddQListButton(L["SKIP_BUTTON"], LineNr, function(self, button)
					APR1[APR.Realm][APR.Name]["hideRidingSkill"] = 1
					APR.QList.QFrames["FS"..LineNr].Button:Hide()
					APR.QList.QFrames[LineNr]:Hide()
				end)
				APR.QList.QFrames[LineNr]:Show()
			end
		end
		...
		if ((steps["ExtraLine"] or steps["ExtraLineText"]) and APR1[APR.Realm][APR.Name]["Settings"]["ShowQList"] == 1 and APR.ZoneTransfer == 0) then
			...
			if (steps["ExtraLineText"] and L[APRExtraText]) then
				APR.QList.QFrames["FS"..LineNr]:SetText(TextWithStars(L[APRExtraText]))
			elseif(steps["ExtraLineText"]) then
				APR.QList.QFrames["FS"..LineNr]:SetText(TextWithStars(APRExtraText))
			end
			if (APRExtraLine == 35) then
				APR.QList.QFrames["FS"..LineNr]:SetText(TextWithStars(L["LOA_INFO_1"]))
				APR.QList.QFrames[LineNr]:Show()
				APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
				local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
				if (APRwidth and APRwidth > 400) then
				...
				LineNr = LineNr + 1
				APR.QList.QFrames["FS"..LineNr]:SetText(TextWithStars(L["LOA_INFO_2"]))
			end
			if (APRExtraLine == 13544) then
				...
				if (itemLink and GetItemCount(itemLink)) then
					APR.QList.QFrames["FS"..LineNr]:SetText(L["KILL_FLEETFOOT"].." "..(".. GetItemCount(itemLink) .."/1))
			...
			if (APRExtraLine == 13595) then
				...
				if (itemLink and GetItemCount(itemLink)) then
					APR.QList.QFrames["FS"..LineNr]:SetText(L["LOOT_WILDFIRE_BOTTLE"].." "..(".. GetItemCount(itemLink) .."/1))
					if (GetItemCount(itemLink) and GetItemCount(itemLink) > 0) then
						APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
						APR.BookingList["UpdateQ"] = 1
						APR.BookingList["PrintQStep"] = 1
			...
			if (APRExtraLine == 14358) then
				...
				if (GetItemInfo(48106) and GetItemCount(GetItemInfo(48106))) then
					if (GetItemCount(GetItemInfo(48106)) and GetItemCount(GetItemInfo(48106)) < 8) then
						APR.QList.QFrames["FS"..LineNr]:SetText("["..LineNr.."] ".. GetItemCount(GetItemInfo(48106)) .."/8 "..L["LOOT_MELONFRUIT"])
						zdsLine = 1
						APR.QList.QFrames[LineNr]:Show()
						APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
						local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
				...
				if (GetItemCount(GetItemInfo(48857))) then
					if (GetItemCount(GetItemInfo(48857)) and GetItemCount(GetItemInfo(48857)) < 10) then
						...
						APR.QList.QFrames["FS"..LineNr]:SetText("["..LineNr.."] ".. GetItemCount(GetItemInfo(48857)) .."/10 "..L["KILL_SATYR_FLESH"])
						zdsLine2 = 1
						APR.QList.QFrames[LineNr]:Show()
						APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
						local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
				...
				if (GetItemInfo(48943) and GetItemCount(GetItemInfo(48943))) then
					if (GetItemCount(GetItemInfo(48943)) and GetItemCount(GetItemInfo(48943)) < 20) then
						...
						APR.QList.QFrames["FS"..LineNr]:SetText("["..LineNr.."] ".. GetItemCount(GetItemInfo(48943)) .."/20 "..L["LOOT_SATYR_SABER"])
						zdsLine3 = 1
						APR.QList.QFrames[LineNr]:Show()
						APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
						local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
			...
			if (APRExtraLine == 25654) then
				if (GetItemInfo(9530) and GetItemCount(GetItemInfo(9530))) then
					APR.QList.QFrames["FS"..LineNr]:SetText(L["LOOT_HARPYS_HORN"].."(".. GetItemCount(GetItemInfo(9530)) .."/1)")
			...
			if (APRExtraLine == 27237) then
				...
				if (itemLink and GetItemCount(itemLink)) then
					APR.QList.QFrames["FS"..LineNr]:SetText(L["OPEN_BAG"])
					if (GetItemCount(itemLink) and GetItemCount(itemLink) > 0) then
						APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
						APR.BookingList["UpdateQ"] = 1
						APR.BookingList["PrintQStep"] = 1
		...
		for i = 2, 4 do
			...
			if ((steps[extraLineTestX]) and APR1[APR.Realm][APR.Name]["Settings"]["ShowQList"] == 1 and APR.ZoneTransfer == 0) then
				...
				if (steps[extraLineTestX]) then
					APR.QList.QFrames["FS"..LineNr]:SetText(TextWithStars(steps[extraLineTestX]))
				end
				APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
				APR.QList.QFrames[LineNr]:Show()
				local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
		...
		if (APR.ActiveQs and APR.ActiveQs[57867] and APR.ZoneTransfer == 0) then
			APR.QList.QBuffFrame:Show()
		else
		...
		if (StepP == "QPart") then
			...
			if (steps["QPartDB"]) then
				...
				for hz=1, getn(steps["QPartDB"]) do
					...
					if (C_QLog.IsQFlaggedCompleted(ZeQID) or APR.ActiveQs[ZeQID]) then
						ZeIDi = ZeQID
						break
					end
				end
			...
			if (steps["QSpecialz"] and APR.ActiveQs["57657-2"]) then
				for i=1,40 do
					...
					if (spellId and spellId == 309806) then
						LineNr = LineNr + 1
						APR.QList.QFrames["FS"..LineNr]:SetText("["..LineNr.."] "..count.."/30 "..L["GORMLINGS_COLLECTED"])
						APR.QList.QFrames[LineNr]:Show()
						local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
						if (APRwidth and APRwidth > 400) then
							APR.QList.QFrames[LineNr]:SetWidth(APRwidth+10)
			...
			if (APR.ActiveQs["57710-2"]) then
				if (Q2Special57710 ~= APR.ActiveQs["57710-2"]) then
					Q2Special57710 = APR.ActiveQs["57710-2"]
					QSpecial57710 = 0
				end
				if (QSpecial57710 == 0) then
					LineNr = LineNr + 1
					APR.QList.QFrames["FS"..LineNr]:SetText(TextWithStars(L["CLICK_ETERNAL_FLAME"]))
					APR.QList.QFrames[LineNr]:Show()
					local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
					if (APRwidth and APRwidth > 400) then
						APR.QList.QFrames[LineNr]:SetWidth(APRwidth+10)
			...
			for APR_index,APR_value in pairs(IdList) do
				for APR_index2,APR_value2 in pairs(APR_value) do
					...
					if (C_QLog.IsQFlaggedCompleted(APR_index) or ((UnitLevel("player") == 121) and APR_BonusObj[APR_index]) or APR1[APR.Realm][APR.Name]["BonusSkips"][APR_index] or APR.BreadCrumSkips[APR_index]) then
						Flagged = Flagged + 1
					elseif (APR.ActiveQs[qid] and APR.ActiveQs[qid] == "C") then
						Flagged = Flagged + 1
					elseif (APR.ActiveQs[qid]) then
						if (APR1[APR.Realm][APR.Name]["Settings"]["ShowQList"] == 1 and APR.ZoneTransfer == 0) then
							LineNr = LineNr + 1
							local ZeTExt
							if (APR.ActiveQs["57713-4"] and UIWidgetTopCenterContainerFrame and UIWidgetTopCenterContainerFrame["widgetFrames"]) then
							...
							local checkpbar = C_QLog.GetQObjectives(APR_index)
						...
						--		APR.QList.QFrames["FS"..LineNr]:SetText("["..LineNr.."] "..GetQProgressBarPercent(APR_index).."/100 "..APR.ActiveQs[qid])
							if (not string.find(APR.ActiveQs[qid], "(.*)(%d+)(.*)") and checkpbar and checkpbar[tonumber(APR_index2)] and checkpbar[tonumber(APR_index2)]["type"] and checkpbar[tonumber(APR_index2)]["type"] == "progressbar") then
								APR.QList.QFrames["FS"..LineNr]:SetText("["..LineNr.."] ["..GetQProgressBarPercent(APR_index).."%] "..APR.ActiveQs[qid])
							elseif (ZeTExt) then
								APR.QList.QFrames["FS"..LineNr]:SetText("["..LineNr.."] "..ZeTExt.."% - "..APR.ActiveQs[qid])
							else
								APR.QList.QFrames["FS"..LineNr]:SetText("["..LineNr.."] "..APR.ActiveQs[qid])
							end
							APR.QList.QFrames[LineNr]:Show()
							APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
							local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
							...
							if (APR_BonusObj[APR_index]) then
								APR.QList.QFrames[LineNr]["BQid"] = APR_index
								APR.QList.QFrames["FS"..LineNr]["Button"]:Show()
							else
								APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
							end
						end
					elseif (not APR.ActiveQs[APR_index] and not MissingQs[APR_index]) then
						if (APR1[APR.Realm][APR.Name]["Settings"]["ShowQList"] == 1 and APR.ZoneTransfer == 0) then
							if (APR_BonusObj[APR_index]) then
								LineNr = LineNr + 1
								APR.QList.QFrames["FS"..LineNr]:SetText(L["DO_BONUS_OBJECTIVE"]..": "..APR_index)
								APR.QList.QFrames[LineNr]:Show()
								APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
								local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
								if (APRwidth and APRwidth > 400) then
								...
								LineNr = LineNr + 1
								APR.QList.QFrames["FS"..LineNr]:SetText(L["ERROR"].." - "..L["MISSING_Q"]..": "..APR_index)
								APR.QList.QFrames[LineNr]:Show()
								APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
								local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
								if (APRwidth and APRwidth > 400) then
		...
		elseif (StepP == "PickUp") then
			...
			if (steps["PickUpDB"]) then
				...
				if (Flagged > 0) then
					...
					if (APR1[APR.Realm][APR.Name]["Settings"]["ShowQList"] == 1 and APR.ZoneTransfer == 0) then
						LineNr = LineNr + 1
						APR.QList.QFrames["FS"..LineNr]:SetText(L["PICK_UP_QS"]..": 1")
						APR.QList.QFrames[LineNr]:Show()
						APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
						local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
						if (APRwidth and APRwidth > 400) then
				...
				for h=1, getn(IdList) do
					...
					Total = Total + 1
					if (not APR.ActiveQs[theqid] and C_QLog.IsQFlaggedCompleted(theqid) == false) then
						NrLeft = NrLeft + 1
					end
				...
				elseif (steps["PickUp2"] and Total2 == Flagged2) then
					...
					if (APR1[APR.Realm][APR.Name]["Settings"]["ShowQList"] == 1 and APR.ZoneTransfer == 0) then
						LineNr = LineNr + 1
						APR.QList.QFrames["FS"..LineNr]:SetText(L["PICK_UP_QS"]..": "..NrLeft)
						APR.QList.QFrames[LineNr]:Show()
						APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
						local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
						if (APRwidth and APRwidth > 400) then
		...
		elseif (StepP == "CRange") then
			IdList = steps["CRange"]
			if (C_QLog.IsQFlaggedCompleted(IdList) or APR.BreadCrumSkips[IdList]) then
				APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
				if (APR1["Debug"]) then
				...
				if (APR1[APR.Realm][APR.Name]["Settings"]["ShowQList"] == 1 and APR.ZoneTransfer == 0) then
					LineNr = LineNr + 1
					APR.QList.QFrames["FS"..LineNr]:SetText(APR.CheckCRangeText())
					APR.QList.QFrames[LineNr]:Show()
--					APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
					APR.QList.QFrames[LineNr]["BQid"] = APR_index
					APR.QList.QFrames["FS"..LineNr]["Button"]:Show()
		...
		elseif (StepP == "TrainRiding") then
			...
			if (C_QLog.IsQFlaggedCompleted(IdList) or (GetSpellBookItemInfo(GetSpellInfo(steps["SpellInTab"])))) then
				APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
				APR.BookingList["UpdateQ"] = 1
				APR.BookingList["PrintQStep"] = 1
			end
		elseif (StepP == "Treasure") then
			...
			if (C_QLog.IsQFlaggedCompleted(IdList)) then
				APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
				if (APR1["Debug"]) then
				...
				if (APR1[APR.Realm][APR.Name]["Settings"]["ShowQList"] == 1 and APR.ZoneTransfer == 0) then
					LineNr = LineNr + 1
					APR.QList.QFrames["FS"..LineNr]:SetText(L["GET_TREASURE"])
					APR.QList.QFrames[LineNr]:Show()
					APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
					local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
					if (APRwidth and APRwidth > 400) then
		...
		elseif (StepP == "DropQ") then
			IdList = steps["DropQ"]
			if (C_QLog.IsQFlaggedCompleted(IdList) or APR.ActiveQs[IdList]) then
				APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
				if (APR1["Debug"]) then
					print("APR.PrintQStep:DropQ:Plus:"..APR1[APR.Realm][APR.Name][APR.ActiveMap])
				end
				APR.BookingList["UpdateQ"] = 1
				APR.BookingList["PrintQStep"] = 1
			end
		elseif (StepP == "Done") then
			...
			if (Total == Flagged) then
				...
				if (APR1[APR.Realm][APR.Name]["Settings"]["ShowQList"] == 1 and APR.ZoneTransfer == 0) then
					LineNr = LineNr + 1
					APR.QList.QFrames["FS"..LineNr]:SetText(L["TURN_IN_Q"]..": "..NrLeft)
					APR.QList.QFrames[LineNr]:Show()
					APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
					local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
					if (APRwidth and APRwidth > 400) then
		...
		elseif (StepP == "WarMode") then
			if (C_QLog.IsQFlaggedCompleted(steps["WarMode"]) or C_PvP.IsWarModeDesired() == true) then
				APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
				APR.BookingList["PrintQStep"] = 1
			else
				if (APR1[APR.Realm][APR.Name]["Settings"]["ShowQList"] == 1 and APR.ZoneTransfer == 0) then
					LineNr = LineNr + 1
					APR.QList.QFrames["FS"..LineNr]:SetText(TextWithStars(L["TURN_ON_WARMODE"], 3))
					APR.QList.QFrames[LineNr]:Show()
					APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
					local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
					if (APRwidth and APRwidth > 400) then
		...
		elseif (StepP == "UseDalaHS") then
			if (C_QLog.IsQFlaggedCompleted(steps["UseDalaHS"])) then
				APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
				APR.BookingList["PrintQStep"] = 1
			else
				if (APR1[APR.Realm][APR.Name]["Settings"]["ShowQList"] == 1 and APR.ZoneTransfer == 0) then
					...
					APR.QList.QFrames["FS"..LineNr]:SetText(L["USE_DALARAN_HEARTHSTONE"])
					APR.QList.QFrames[LineNr]:Show()
					APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
					local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
					if (APRwidth and APRwidth > 400) then
		...
		elseif (StepP == "UseGarrisonHS") then
			if (C_QLog.IsQFlaggedCompleted(steps["UseGarrisonHS"])) then
				APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
				APR.BookingList["PrintQStep"] = 1
			else
				if (APR1[APR.Realm][APR.Name]["Settings"]["ShowQList"] == 1 and APR.ZoneTransfer == 0) then
					LineNr = LineNr + 1
					APR.QList.QFrames["FS"..LineNr]:SetText(L["USE_GARRISON_HEARTHSTONE"])
					APR.QList.QFrames[LineNr]:Show()
					APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
		...
		elseif (StepP == "ZonePick") then
			if (APR1[APR.Realm][APR.Name]["Settings"]["ShowQList"] == 1 and APR.ZoneTransfer == 0) then
				LineNr = LineNr + 1
				APR.QList.QFrames["FS"..LineNr]:SetText(L["PICK_ZONE"])
				APR.QList.QFrames[LineNr]:Show()
				APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
				local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
				if (APRwidth and APRwidth > 400) then
		...
		elseif (StepP == "SetHS") then
			if (APR1[APR.Realm][APR.Name]["Settings"]["ShowQList"] == 1 and APR.ZoneTransfer == 0) then
				LineNr = LineNr + 1
				APR.QList.QFrames["FS"..LineNr]:SetText(L["SET_HEARTHSTONE"])
				APR.QList.QFrames[LineNr]:Show()
				APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
				local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
				if (APRwidth and APRwidth > 400) then
			...
			if (C_QLog.IsQFlaggedCompleted(steps["SetHS"])) then
				APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
				APR.BookingList["PrintQStep"] = 1
		...
		elseif (StepP == "UseHS") then
			if (APR1[APR.Realm][APR.Name]["Settings"]["ShowQList"] == 1 and APR.ZoneTransfer == 0) then
				LineNr = LineNr + 1
				APR.QList.QFrames["FS"..LineNr]:SetText(L["USE_HEARTHSTONE"])
				APR.QList.QFrames[LineNr]:Show()
				APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
				local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
				if (APRwidth and APRwidth > 400) then
			...
			if (C_QLog.IsQFlaggedCompleted(steps["UseHS"])) then
				APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
				APR.BookingList["PrintQStep"] = 1
			end
		elseif (StepP == "GetFP") then
			if (APR1[APR.Realm][APR.Name]["Settings"]["ShowQList"] == 1 and APR.ZoneTransfer == 0) then
				APR.FP.GoToZone = nil
				LineNr = LineNr + 1
				APR.QList.QFrames["FS"..LineNr]:SetText(L["GET_FLIGHPATH"])
				APR.QList.QFrames[LineNr]:Show()
				APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
				local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
				if (APRwidth and APRwidth > 400) then
			...
			if (C_QLog.IsQFlaggedCompleted(steps["GetFP"])) then
				APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
				APR.BookingList["PrintQStep"] = 1
			end
		elseif (StepP == "UseFlightPath") then
			if (APR1[APR.Realm][APR.Name]["Settings"]["ShowQList"] == 1 and APR.ZoneTransfer == 0) then
				...
				if (steps["Boat"]) then
					if (steps["Name"]) then
						APR.QList.QFrames["FS"..LineNr]:SetText(L["USE_BOAT"]..": "..steps["Name"])
					else
						APR.QList.QFrames["FS"..LineNr]:SetText(L["USE_BOAT"])
					end

				else
					if (steps["Name"]) then
						APR.QList.QFrames["FS"..LineNr]:SetText(L["USE_FLIGHTPATH"]..": "..steps["Name"])
					else
						APR.QList.QFrames["FS"..LineNr]:SetText(L["USE_FLIGHTPATH"])
					end
				end
				APR.QList.QFrames[LineNr]:Show()
				APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
			...
			if (C_QLog.IsQFlaggedCompleted(steps["UseFlightPath"])) then
				APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
				APR.BookingList["PrintQStep"] = 1
			end
		elseif (StepP == "QaskPopup") then
			if (C_QLog.IsQFlaggedCompleted(steps["QaskPopup"])) then
				APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
				APR.BookingList["PrintQStep"] = 1
			else
				APR_QAskPopWanted()
		...
		elseif (StepP == "QPartPart") then
			...
			elseif (steps and steps["TrigText"]) then
				for APR_index,APR_value in pairs(steps["QPartPart"]) do
					for APR_index2,APR_value2 in pairs(APR_value) do
						if (APR.ActiveQs[APR_index.."-"..tonumber(APR_index2)]) then
							if (string.find(APR.ActiveQs[APR_index.."-"..tonumber(APR_index2)], steps["TrigText"])) then
								APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
								APR.BookingList["PrintQStep"] = 1
							elseif (steps["TrigText2"] and string.find(APR.ActiveQs[APR_index.."-"..tonumber(APR_index2)], steps["TrigText2"])) then
								APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
								APR.BookingList["PrintQStep"] = 1
							elseif (steps["TrigText3"] and string.find(APR.ActiveQs[APR_index.."-"..tonumber(APR_index2)], steps["TrigText3"])) then
								APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
								APR.BookingList["PrintQStep"] = 1
							end
						end
		...
		if (steps["DroppableQ"] and not C_QLog.IsQFlaggedCompleted(steps["DroppableQ"]["Qid"]) and not APR.ActiveQs[steps["DroppableQ"]["Qid"]]) then
			if (APR1[APR.Realm][APR.Name]["Settings"]["ShowQList"] == 1 and APR.ZoneTransfer == 0) then
				LineNr = LineNr + 1
				local MobName = steps["DroppableQ"]["Text"]
				if (APR.NPCList[steps["DroppableQ"]["MobId"]]) then
					MobName = APR.NPCList[steps["DroppableQ"]["MobId"]]
				end
				APR.QList.QFrames["FS"..LineNr]:SetText("[".. LineNr .."] "..MobName..L["Q_DROP"])
				APR.QList.QFrames[LineNr]:Show()
				APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
				local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
				if (APRwidth and APRwidth > 400) then
		...
		if (steps["Fillers"] and APR1[APR.Realm][APR.Name]["Settings"]["ShowQList"] == 1) then
			...
			for APR_index,APR_value in pairs(IdList) do
				for APR_index2,APR_value2 in pairs(APR_value) do
					if (C_QLog.IsQFlaggedCompleted(APR_index) == false and not APR1[APR.Realm][APR.Name]["BonusSkips"][APR_index]) then
						if ((UnitLevel("player") ~= 121) or (UnitLevel("player") == 121 and not APR_BonusObj[APR_index])) then
							local qid = APR_index.."-"..APR_index2
							if (APR.ActiveQs[qid] and APR.ActiveQs[qid] == "C") then
							elseif (APR.ActiveQs[qid] and APR.ZoneTransfer == 0) then
								LineNr = LineNr + 1
								local checkpbar = C_QLog.GetQObjectives(APR_index)
								if (not string.find(APR.ActiveQs[qid], "(.*)(%d+)(.*)") and checkpbar and checkpbar[tonumber(APR_index2)] and checkpbar[tonumber(APR_index2)]["type"] and checkpbar[tonumber(APR_index2)]["type"] == "progressbar") then
									APR.QList.QFrames["FS"..LineNr]:SetText("["..LineNr.."] ["..GetQProgressBarPercent(APR_index).."%] "..APR.ActiveQs[qid])
		...
		if (APR.QListShown ~= LineNr) then
			if (APR.QListShown > LineNr) then
				local FrameHideNr = APR.QListShown - LineNr
				local NewLine = LineNr
				local CLi
				for CLi = 1, FrameHideNr do
					NewLine = NewLine + CLi
					if (APR.QList.QFrames[NewLine]) then
						APR.QList.QFrames[NewLine]:Hide()
						if (not InCombatLockdown()) then
							APR.QList.QFrames["FS"..NewLine]["Button"]:Hide()
							APR.QList2["BF"..NewLine]:Hide()
						end
						if (APR1["Debug"]) then
							print("Hide:"..NewLine)
						end
		...
		APR.QListShown = LineNr
		APR.BookingList["SetQPTT"] = 1
		if (APR.ZoneQOrder:IsShown() == true) then
			APR.BookingList["UpdateZoneQOrderListL"] = 1
		end
	elseif (APRWhereToGo and APR1[APR.Realm][APR.Name]["Settings"]["ShowQList"] == 1 and APR.ZoneTransfer == 0) then
		LineNr = LineNr + 1
		APR.QList.QFrames["FS"..LineNr]:SetText(TextWithStars("APR: "..L["GO_TO"].." ".. APRWhereToGo, 2, true))
		APR.QList.QFrames[LineNr]:Show()
		APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
		local APRwidth = APR.QList.QFrames["FS"..LineNr]:GetStringWidth()
		if (APRwidth and APRwidth > 400) then
			...
			if (APR.SetButtonVar) then
					...
					for APR_index,APR_value in pairs(steps["Button"]) do
						...
						if (APR_index2 == APR_index or steps["UseHS"] or steps["UseGarrisonHS"]) then
							...
							if (itemName and string.sub(itemName, 1, 1) and itemTexture) then
								...
								APR.QList2["BF"..APR_value2]["APR_Buttonptex"]:SetTexture(itemTexture)
								APR.QList2["BF"..APR_value2]["APR_Buttonntex"]:SetTexture(itemTexture)
								APR.QList2["BF"..APR_value2]["APR_Button"]:SetNormalTexture(itemTexture)
								APR.QList2["BF"..APR_value2]["APR_Button"]:SetText("")
								APR.QList2["BF"..APR_value2]["APR_Button"]:SetAttribute("type", "item");
								APR.QList2["BF"..APR_value2]["APR_Button"]:SetAttribute("item", "item:"..APR_value);
								APR.QList2["BF"..APR_value2]["APR_Button"]:SetScript("OnEnter", function(self) GameTooltip:SetOwner(self, "ANCHOR_CURSOR"); GameTooltip:SetItemByID(APR_value); GameTooltip:Show() end)
								APR.QList2["BF"..APR_value2]["APR_Button"]:SetScript("OnLeave", function(self) GameTooltip:Hide() end)
								if (GetItemCount(itemLink) and GetItemCount(itemLink) > 0) then
									APR.QList2["BF"..APR_value2]:Show()
								else
									APR.QList2["BF"..APR_value2]:Hide()
								end
								local Topz = APR1[APR.Realm][APR.Name]["Settings"]["left"]
								local Topz2 = APR1[APR.Realm][APR.Name]["Settings"]["top"]
								APR.QList20:SetPoint("TOPLEFT", UIParent, "TOPLEFT", Topz, Topz2)
								APR.QList2["BF"..APR_value2]:SetPoint("BOTTOMLEFT", APR.QList21, "BOTTOMLEFT",0,-((APR_value2 * 38)+APR_value2))
								if (not APR.ButtonVisual) then
									APR.ButtonVisual = {}
								end
								local _, spellID = GetItemSpell(APR_value)
								if (spellID) then
									APR.QStepList[APR.ActiveMap][CurStep]["ButtonSpellId"] = { [spellID] = APR_index }
								end
				...
				if (APR.Dinged100 == 1 and APR.Dinged100nr > 0) then
					...
					if (itemName and string.sub(itemName, 1, 1) and itemTexture) then
						...
						APR.QList2["BF"..APR.Dinged100nr]["APR_Buttonptex"]:SetTexture(itemTexture)
						APR.QList2["BF"..APR.Dinged100nr]["APR_Buttonntex"]:SetTexture(itemTexture)
						APR.QList2["BF"..APR.Dinged100nr]["APR_Button"]:SetNormalTexture(itemTexture)
						APR.QList2["BF"..APR.Dinged100nr]["APR_Button"]:SetText("")
						APR.QList2["BF"..APR.Dinged100nr]["APR_Button"]:SetAttribute("type", "item");
						APR.QList2["BF"..APR.Dinged100nr]["APR_Button"]:SetAttribute("item", "item:6948");
						APR.QList2["BF"..APR.Dinged100nr]:Show()
						APR.QList2["BF"..APR.Dinged100nr]["APR_Button"]:SetScript("OnEnter", function(self) GameTooltip:SetOwner(self, "ANCHOR_CURSOR"); GameTooltip:SetItemByID(6948); GameTooltip:Show() end)
						APR.QList2["BF"..APR.Dinged100nr]["APR_Button"]:SetScript("OnLeave", function(self) GameTooltip:Hide() end)
						local Topz = APR1[APR.Realm][APR.Name]["Settings"]["left"]
						local Topz2 = APR1[APR.Realm][APR.Name]["Settings"]["top"]
						APR.QList20:SetPoint("TOPLEFT", UIParent, "TOPLEFT", Topz, Topz2)
						APR.QList2["BF"..APR.Dinged100nr]:SetPoint("BOTTOMLEFT", APR.QList21, "BOTTOMLEFT",0,-((APR.Dinged100nr * 38)+APR.Dinged100nr))
						if (not APR.ButtonVisual) then
							APR.ButtonVisual = {}
						end
...
local function APR_UpdateQ()
	if (APR1["Debug"]) then
		print("Function: APR_UpdateQ()")
	end
	local i = 1
	local UpdateQPart = 0
	...
	while C_QLog.GetTitleForLogIndex(i) do
		local ZeInfoz = C_QLog.GetInfo(i)
		if (ZeInfoz) then
			local QID = ZeInfoz["QID"]
			if (QID > 0) then
				local isHeader = ZeInfoz["isHeader"]
				local QTitle = C_QLog.GetTitleForQID(QID)
				local isComplete = C_QLog.IsComplete(QID)
				if (not isHeader) then
					APRQNames[QID] = QTitle
					local numObjectives = C_QLog.GetNumQObjectives(QID)
					if (not APR.ActiveQs[QID]) then
						if (APR1["Debug"]) then
							print("New Q:"..QID)
						end
					end
					...
					if (numObjectives == 0) then
						if (isComplete == 1) then
							APR.ActiveQs[QID.."-".."1"] = "C"
						else
							APR.ActiveQs[QID.."-".."1"] = QTitle
						end
					else
						local ZeObject = C_QLog.GetQObjectives(QID)
						for h=1, numObjectives do
							...
							if (finished == 1) then
								if (APR.ActiveQs[QID.."-"..h] and APR.ActiveQs[QID.."-"..h] ~= "C") then
									if (APR1["Debug"]) then
										print("Update:".."C")
									end
									Update = 1
								end
								APR.ActiveQs[QID.."-"..h] = "C"
							elseif ((select(2,GetQObjectiveInfo(QID, 1, false)) == "progressbar") and text) then
								if (not APR.ProgressbarIgnore[QID.."-"..h]) then
									local APR_Mathstuff = tonumber(GetQProgressBarPercent(QID))
									APR_Mathstuff = floor((APR_Mathstuff + 0.5))
									text = "["..APR_Mathstuff.."%] " .. text
									if (not APR.ActiveQs[QID.."-"..h]) then
										if (APR1["Debug"]) then
											print("New1:"..text)
										end
									end
								end
								if (APR.ActiveQs[QID.."-"..h] and APR.ActiveQs[QID.."-"..h] ~= text) then
									if (APR1["Debug"]) then
										print("Update:"..text)
									end
									Update = 1
									APR.ActiveQs[QID.."-"..h] = text
								else
									APR.ActiveQs[QID.."-"..h] = text
								end
							else
...
function APR.CreateMacro()
	...
	if APR_hasSpace then
		if not isFound and not InCombatLockdown() then
			CreateMacro("APR_MACRO","INV_MISC_QIONMARK","/script print('no button yet')",nil,nil)
		end
	else
...
function APR.MacroUpdater2(macroSlot,itemName,APRextra)
	...
	if (itemName) then
		if (itemName == 123123123) then
			EditMacro(macroSlot, "APR_MACRO","INV_MISC_QIONMARK","#showtooltip\n/click ExtraActionButton1",nil,nil)
		...
		EditMacro(macroSlot, "APR_MACRO","INV_MISC_QIONMARK","/script print('no button yet')",nil,nil)
	end
end
...
local function APR_QStepIds()
	if (APR.QStepList[APR.ActiveMap]) then
		local CurStep = APR1[APR.Realm][APR.Name][APR.ActiveMap]
		if (CurStep and APR.QStepList[APR.ActiveMap][CurStep]) then
			local steps = APR.QStepList[APR.ActiveMap][CurStep]
...
local function APR_RemoveQ(QID)
	APR.ActiveQs[QID] = nil
	for APR_index,APR_value in pairs(APR.ActiveQs) do
		if (string.find(APR_index, "(.*)-(.*)")) then
			local _, _, APR_First, APR_Rest = string.find(APR_index, "(.*)-(.*)")
			if (tonumber(APR_First) == QID) then
				APR.ActiveQs[APR_index] = nil
			end
		end
	end
	local IdList, StepP = APR_QStepIds()
	if (StepP == "Done") then
		local NrLeft = 0
		for APR_index,APR_value in pairs(IdList) do
			if (C_QLog.IsQFlaggedCompleted(APR_value) or QID == APR_value) then
			else
				NrLeft = NrLeft + 1
			end
		end
		if (NrLeft == 0) then
			APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
			if (APR1["Debug"]) then
				print("APR.RemoveQ:Plus"..APR1[APR.Realm][APR.Name][APR.ActiveMap])
			end
			APR.BookingList["UpdateQ"] = 1
		end
	end
...
local function APR_AddQ(QID)
	APR.ActiveQs[QID] = "P"
	local IdList, StepP = APR_QStepIds()
	if (StepP == "PickUp") then
		local NrLeft = 0
		for APR_index,APR_value in pairs(IdList) do
			if (not APRQNames[APR_value]) then
				APRQNames[APR_value] = 1
			end
			if (not APR.ActiveQs[APR_value]) then
				NrLeft = NrLeft + 1
			end
		end
		if (NrLeft == 0) then
			APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
			if (APR1["Debug"]) then
				print("APR.AddQ:Plus"..APR1[APR.Realm][APR.Name][APR.ActiveMap])
			end
			APR.BookingList["UpdateQ"] = 1
		end
	end
...
local function APR_UpdateMapId()
	...
	if (APR.ActiveQs and APR.ActiveQs[59974] and APR.ActiveMap == 1536) then
		APR.ActiveMap = 1670
	end
	...
	--end
--	local CurStep = APR1[APR.Realm][APR.Name][APR.ActiveMap]
	--if (CurStep and APR.QStepList and APR.QStepList[APR.ActiveMap] and APR.QStepList[APR.ActiveMap][CurStep]) then
	--else
	--	APR.BookingList["ClosedSettings"] = 1
	--end
	if (APR.QStepListListingZone) then
...
local function APR_CheckZonePick()
	if (APR.ActiveMap == 862) then
		if (C_QLog.IsQFlaggedCompleted(50963) == false and (APR.ActiveQs[47514] or C_QLog.IsQFlaggedCompleted(47514) == true)) then
			APR.BookingList["UpdateMapId"] = 1
			APR.BookingList["PrintQStep"] = 1
		elseif ((APR.ActiveQs[47513] or C_QLog.IsQFlaggedCompleted(47513) == true) and C_QLog.IsQFlaggedCompleted(47315) == false) then
			APR.BookingList["UpdateMapId"] = 1
			APR.BookingList["PrintQStep"] = 1
		elseif ((APR.ActiveQs[47512] or C_QLog.IsQFlaggedCompleted(47512) == true) and C_QLog.IsQFlaggedCompleted(47105) == false) then
			APR.BookingList["UpdateMapId"] = 1
			APR.BookingList["PrintQStep"] = 1
		elseif (C_QLog.IsQFlaggedCompleted(47105) == true and C_QLog.IsQFlaggedCompleted(47315) == true and C_QLog.IsQFlaggedCompleted(50963) == true) then
			APR.BookingList["UpdateMapId"] = 1
			APR.BookingList["PrintQStep"] = 1
		end
	end
end
local function APR_AcceptQer()
	AcceptQ()
end
local function APR_CheckDistance()
	local CurStep = APR1[APR.Realm][APR.Name][APR.ActiveMap]
	if (CurStep and APR.QStepList[APR.ActiveMap] and APR.QStepList[APR.ActiveMap][CurStep]) then
		if (APR.QStepList[APR.ActiveMap][CurStep]["CRange"]) then
			APR.ArrowFrame.Button:Show()
			local plusnr = CurStep
			local Distancenr = 0
			local testad = true
			if (APR.QStepList[APR.ActiveMap][CurStep]["NoExtraRange"]) then
				testad = false
			end
			while testad do
				local oldx = APR.QStepList[APR.ActiveMap][plusnr]["TT"]["x"]
				local oldy = APR.QStepList[APR.ActiveMap][plusnr]["TT"]["y"]
				plusnr = plusnr + 1
				if (APR.QStepList[APR.ActiveMap][plusnr] and APR.QStepList[APR.ActiveMap][plusnr]["CRange"]) then
					local newx = APR.QStepList[APR.ActiveMap][plusnr]["TT"]["x"]
					local newy = APR.QStepList[APR.ActiveMap][plusnr]["TT"]["y"]
					local deltaX, deltaY = oldx - newx, newy - oldy
					local distance = (deltaX * deltaX + deltaY * deltaY)^0.5
					Distancenr = Distancenr + distance
				else
					if (APR.QStepList[APR.ActiveMap][plusnr] and APR.QStepList[APR.ActiveMap][plusnr]["TT"]) then
						local newx = APR.QStepList[APR.ActiveMap][plusnr]["TT"]["x"]
						local newy = APR.QStepList[APR.ActiveMap][plusnr]["TT"]["y"]
						local deltaX, deltaY = oldx - newx, newy - oldy
						local distance = (deltaX * deltaX + deltaY * deltaY)^0.5
...
local function APR_SetQPTT()
	...
	local CurStep = APR1[APR.Realm][APR.Name][APR.ActiveMap]
	if (QNumberLocal ~= CurStep and APR.QStepList and APR.QStepList[APR.ActiveMap] and APR.QStepList[APR.ActiveMap][CurStep] and APR.QStepList[APR.ActiveMap][CurStep]["TT"]) then
		APR.ArrowActive = 1
		APR.ArrowActive_X = APR.QStepList[APR.ActiveMap][CurStep]["TT"]["x"]
		APR.ArrowActive_Y = APR.QStepList[APR.ActiveMap][CurStep]["TT"]["y"]
...
local function APR_PosTest()
	...
	elseif (APR1 and APR1[APR.Realm][APR.Name] and APR1[APR.Realm][APR.Name]["Settings"] and APR1[APR.Realm][APR.Name]["Settings"]["ShowArrow"] == 0) then
		...
		local CurStep = APR1[APR.Realm][APR.Name][APR.ActiveMap]
		if (APR.QStepList and APR.QStepList[APR.ActiveMap] and APR.QStepList[APR.ActiveMap][CurStep] and APR.QStepList[APR.ActiveMap][CurStep]["AreaTriggerZ"]) then
			x = APR.QStepList[APR.ActiveMap][CurStep]["AreaTriggerZ"]["x"]
			y = APR.QStepList[APR.ActiveMap][CurStep]["AreaTriggerZ"]["y"]
			local deltaX, deltaY = d_x - x, y - d_y
			local distance = (deltaX * deltaX + deltaY * deltaY)^0.5
			if (APR.QStepList[APR.ActiveMap][CurStep]["AreaTriggerZ"]["R"] > distance) then
				APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
				QNumberLocal = 0
				APR.BookingList["UpdateQ"] = 1
				APR.BookingList["PrintQStep"] = 1
		...
		if (((APR.ArrowActive == 0) or (APR.ArrowActive_X == 0) or (IsInInstance()) or not APR.QStepList) or (APR.QStepList and APR.QStepList[APR.ActiveMap] and APR.QStepList[APR.ActiveMap][CurStep] and APR.QStepList[APR.ActiveMap][CurStep]["NoArrows"])) then
			...
			if (d_x and d_y and GetPlayerFacing()) then
				...
				if (CurStep and APR.ActiveMap and APR.QStepList[APR.ActiveMap] and APR.QStepList[APR.ActiveMap][CurStep]) then
					if (APR.QStepList[APR.ActiveMap][CurStep]["Trigger"]) then
						local d_y, d_x = UnitPosition("player")
						local APR_ArrowActive_Trigger_X = APR.QStepList[APR.ActiveMap][CurStep]["Trigger"]["x"]
						local APR_ArrowActive_Trigger_Y = APR.QStepList[APR.ActiveMap][CurStep]["Trigger"]["y"]
						...
						APR_ArrowActive_TrigDistance = APR.QStepList[APR.ActiveMap][CurStep]["Range"]
						if (APR.QStepList[APR.ActiveMap][CurStep]["HIDEME"]) then
							APR.ArrowActive = 0
						end
					end
				end
...
local function APR_LoopBookingFunc()
	...
	elseif (APR.BookingList["AcceptQ"]) then
		if (APR1["Debug"]) then
			print("LoopBookingFunc:AcceptQ")
		end
		APR.BookingList["AcceptQ"] = nil
		C_Timer.After(0.2, APR_AcceptQer)
		TestaAPR = "AcceptQ"
	elseif (APR.BookingList["CompleteQ"]) then
		if (APR1["Debug"]) then
			print("LoopBookingFunc:CompleteQ")
		end
		APR.BookingList["CompleteQ"] = nil
		CompleteQ()
		TestaAPR = "CompleteQ"
	...
	elseif (APR.BookingList["AddQ"]) then
		if (APR1["Debug"]) then
			print("LoopBookingFunc:AddQ:"..APR1[APR.Realm][APR.Name][APR.ActiveMap])
		end
		APR_AddQ(APR.BookingList["AddQ"])
		APR.BookingList["AddQ"] = nil
		TestaAPR = "AddQ"
	elseif (APR.BookingList["RemoveQ"]) then
		if (APR1["Debug"]) then
			print("LoopBookingFunc:RemoveQ:"..APR1[APR.Realm][APR.Name][APR.ActiveMap])
		end
		APR_RemoveQ(APR.BookingList["RemoveQ"])
		APR.BookingList["RemoveQ"] = nil
		APR.BookingList["UpdateMapId"] = 1
		APR.BookingList["PrintQStep"] = 1
		TestaAPR = "RemoveQ"
	elseif (APR.BookingList["UpdateQ"]) then
		if (APR1["Debug"]) then
			print("LoopBookingFunc:UpdateQ:")
		end
		APR.BookingList["UpdateQ"] = nil
		APR_UpdateQ()
		TestaAPR = "UpdateQ"
	...
	elseif (APR.BookingList["UpdateZoneQOrderListL"]) then
		APR.UpdateZoneQOrderList("LoadIn")
		APR.BookingList["UpdateZoneQOrderListL"] = nil
	...
	elseif (APR.BookingList["ButtonSpellidchk"]) then
		for APR_index,APR_value in pairs(APR.BookingList["ButtonSpellidchk"]) do
			if (APR_value) then
				...
				if (duration and duration > 0 and APR_index and APR.QList2 and APR.QList2["BF"..APR_index] and APR.QList2["BF"..APR_index]["APR_ButtonCD"]) then
					APR.QList2["BF"..APR_index]["APR_ButtonCD"]:SetCooldown(GetTime(), duration)
				end
			end
		end
...
local function APR_PopupFunc()
	if (GetNumAutoQPopUps() > 0) then
		local QID, popUpType = GetAutoQPopUp(1)
...
function APR_UpdQThing()
	if (UnitGUID("target") and UnitName("target")) then
		local guid, name = UnitGUID("target"), UnitName("target")
		local type, zero, server_id, instance_id, zone_uid, npc_id, spawn_uid = strsplit("-",guid);
		if (npc_id and name) then
			if (APR.ActiveQs and APR.ActiveQs["55981-3"] and APR.ActiveQs["55981-3"] ~= "C" and tonumber(npc_id) == 153580) then
				DoEmote("hug")
			elseif (APR.ActiveQs and APR.ActiveQs["55981-4"] and APR.ActiveQs["55981-4"] ~= "C" and tonumber(npc_id) == 153580) then
				DoEmote("wave")
			elseif (APR.ActiveQs and APR.ActiveQs["59978-4"] and APR.ActiveQs["59978-4"] ~= "C" and tonumber(npc_id) == 153580) then
				DoEmote("wave")
			end
		end
	end
	...
	Updateblock = 0
	if (APR1["Debug"]) then
		print("Extra UpdQThing")
	end
end
function APR_UpdateMapId()
	APR.BookingList["UpdateMapId"] = 1
end
	...
	for i=1,20 do
		...
		if (spellId and name) then
			if (spellId == 311103) then
				APR.QBuff[1] = 1
				APR.QList.QBuffFrame.Traps.texture:SetColorTexture(0.1,0.5,0.1,1)
			end
			if (spellId == 311107) then
				APR.QBuff[2] = 1
				APR.QList.QBuffFrame.Traps2.texture:SetColorTexture(0.1,0.5,0.1,1)
			end
			if (spellId == 311058) then
				APR.QBuff[3] = 1
				APR.QList.QBuffFrame.Traps3.texture:SetColorTexture(0.1,0.5,0.1,1)
			end
		end
	end
end
...
APR_QH_EventFrame:RegisterEvent ("UNIT_Q_LOG_CHANGED")
...
APR_QH_EventFrame:RegisterEvent ("Q_PROGRESS")
APR_QH_EventFrame:RegisterEvent ("Q_DETAIL")
APR_QH_EventFrame:RegisterEvent ("Q_COMPLETE")
APR_QH_EventFrame:RegisterEvent ("Q_FINISHED")
APR_QH_EventFrame:RegisterEvent ("TAXIMAP_OPENED")
APR_QH_EventFrame:RegisterEvent ("MERCHANT_SHOW")
APR_QH_EventFrame:RegisterEvent ("Q_GREETING")
APR_QH_EventFrame:RegisterEvent ("ITEM_PUSH")
APR_QH_EventFrame:RegisterEvent ("Q_AUTOCOMPLETE")
...
APR_QH_EventFrame:RegisterEvent ("REQ_CEMETERY_LIST_RESPONSE")
...
APR_QH_EventFrame:SetScript("OnEvent", function(self, event, ...)
	if (event=="UPDATE_UI_WIDGET") then
		if (APR.ActiveQs and APR.ActiveQs["57713-4"]) then
			APR.BookingList["PrintQStep"] = 1
		end
	end
	if (event=="AJ_REFRESH_DISPLAY") then
	end
	if (event=="REQ_CEMETERY_LIST_RESPONSE") then
		APR.BookingList["UpdateMapId"] = 1
	...
	if (event=="LEARNED_SPELL_IN_TAB") then
		...
		local CurStep = APR1[APR.Realm][APR.Name][APR.ActiveMap]
		local steps
		if (CurStep and APR.QStepList and APR.QStepList[APR.ActiveMap]) then
			steps = APR.QStepList[APR.ActiveMap][CurStep]
		end
	...
	if (event=="Q_LOG_UPDATE") then
		C_Timer.After(0.1, APR_UpdQThing)
	end
	if (event=="UNIT_AURA") then
		...
		if (steps and steps["QSpecialz"] and APR.ActiveQs and APR.ActiveQs["57657-2"]) then
			APR.BookingList["PrintQStep"] = 1
		end
		...
		if (APR.QBuff[1] == 1 or APR.QBuff[2] == 1 or APR.QBuff[3] == 1) then
			local gotbuff1 = 0
			local gotbuff2 = 0
			local gotbuff3 = 0
			for i=1,20 do
			...
			if (APR.QBuff[1] == 1) then
				if (gotbuff1 == 0) then
					APR.QBuff[1] = 0
					APR.QList.QBuffFrame.Traps.texture:SetColorTexture(0.5,0.1,0.1,1)
				end
			end
			if (APR.QBuff[2] == 1) then
				if (gotbuff2 == 0) then
					APR.QBuff[2] = 0
					APR.QList.QBuffFrame.Traps2.texture:SetColorTexture(0.5,0.1,0.1,1)
				end
			end
			if (APR.QBuff[3] == 1) then
				if (gotbuff3 == 0) then
					APR.QBuff[3] = 0
					APR.QList.QBuffFrame.Traps3.texture:SetColorTexture(0.5,0.1,0.1,1)
				end
			end
		end
		if (arg1 == "player" and APR.ActiveQs and APR.ActiveQs[57867]) then
			APR.CheckQBuffs()
			C_Timer.After(2, APR.CheckQBuffs)
		end
	end
	...
	if (event=="PLAYER_CHOICE_UPDATE") then
		...
		if (Choizs) then
			...
			local QionText = Choizs["QionText"]
			...
			if (numOptions and numOptions > 1 and steps and steps["Brewery"]) then
				...
				for CLi = 1, numOptions do
					...
					if (steps["Brewery"] == optionID) then
						--C_PlayerChoice.SendQChoiceResponse(GetQChoiceOptionInfo(CLi))
		...
		-- 	local CLi
		-- 	for CLi = 1, numOptions do
		-- 		local optionID, buttonText, description, artFile = GetQChoiceOptionInfo(CLi)
		-- 		if (steps["PickUpSpecial"] == optionID) then
		-- 			SendQChoiceResponse(GetQChoiceOptionInfo(CLi))
		-- 			APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
		-- 			APR.BookingList["UpdateQ"] = 1
		-- 			APR.BookingList["PrintQStep"] = 1
		-- 			break
	...
	if (event=="Q_GREETING") then
		...
		local numAvailableQs = 0;
		local numActiveQs = 0;
		local lastActiveQ = 0
		local lastAvailableQ = 0;
		numAvailableQs = GetNumAvailableQs();
		numActiveQs = GetNumActiveQs();
		if numAvailableQs > 0 or numActiveQs > 0 then
			...
			if (lastAvailableQ > numAvailableQs) then
				lastAvailableQ = 1;
			end
			for i = lastAvailableQ, numAvailableQs do
				lastAvailableQ = i;
				if (APR1[APR.Realm][APR.Name]["Settings"]["AutoAccept"] == 1 and not IsControlKeyDown()) then
					SelectAvailableQ(i);
				end
		...
		if lastActiveQ > numActiveQs then
			lastActiveQ = 1;
		end
		if (APR1[APR.Realm][APR.Name]["Settings"]["AutoHandIn"] == 1 and not IsControlKeyDown()) then
			local TempQList = {}
	...
	if (event=="UNIT_SPELLCAST_SUCCEEDED") then
		...
		if (arg1 == "player") then
			local CurStep = APR1[APR.Realm][APR.Name][APR.ActiveMap]
			local steps
			if (CurStep and APR.QStepList and APR.ActiveMap and APR.QStepList[APR.ActiveMap] and APR.QStepList[APR.ActiveMap][CurStep]) then
				steps = APR.QStepList[APR.ActiveMap][CurStep]
			end

			if (QSpecial57710 == 0 and arg3 == 310061) then
				QSpecial57710 = 1
				APR.BookingList["PrintQStep"] = 1
			end
	...
	if (event=="HEARTHSTONE_BOUND") then
		...
		APR1[APR.Realm][APR.Name]["HSLoc"] = Map
	...
	if (event=="Q_ACCEPTED") then
		...
		if (arg2 and arg2 > 0 and not APR.ActiveQs[arg2]) then
			APR.BookingList["AddQ"] = arg2
		end
		local CurStep = APR1[APR.Realm][APR.Name][APR.ActiveMap]
		if (CurStep and APR.QStepList and APR.ActiveMap and APR.QStepList[APR.ActiveMap] and APR.QStepList[APR.ActiveMap][CurStep]) then
			local steps = APR.QStepList[APR.ActiveMap][CurStep]
			...
			if (steps and steps["LoaPick"] and steps["LoaPick"] == 123 and (APR.ActiveQs[47440] or APR.ActiveQs[47439])) then
				APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
				APR.BookingList["PrintQStep"] = 1
			end
		end
	...
	if (event=="Q_REMOVED") then
		...
		APR1[APR.Realm][APR.Name]["QCounter2"] = APR1[APR.Realm][APR.Name]["QCounter2"] + 1
	end
	if (event=="UNIT_Q_LOG_CHANGED") then
		local arg1, arg2, arg3, arg4, arg5 = ...;
		if (arg1 == "player" and Updateblock == 0) then
			Updateblock = 1
			C_Timer.After(1, APR_UpdQThing)
		end
	end
	if (event=="ZONE_CHANGED") then
		QNumberLocal = 0
		if (APR.ZoneTransfer == 0) then
			C_Timer.After(2, APR_UpdateMapId)
	...
	if (event=="GOSSIP_SHOW") then
		...
		local CurStep = APR1[APR.Realm][APR.Name][APR.ActiveMap]
		if (CurStep and APR.QStepList[APR.ActiveMap] and APR.QStepList[APR.ActiveMap][CurStep] and APR1[APR.Realm][APR.Name]["Settings"]["AutoGossip"] == 1 and not IsControlKeyDown()) then
			local steps = APR.QStepList[APR.ActiveMap][CurStep]
			if (UnitGUID("target") and string.find(UnitGUID("target"), "(.*)-(.*)")) then
				local type, zero, server_id, instance_id, zone_uid, npc_id, spawn_uid = strsplit("-",UnitGUID("target"))
			...
			elseif (steps and steps["Gossip"] and APR1[APR.Realm][APR.Name]["Settings"]["AutoGossip"] == 1 and not IsControlKeyDown()) then
				...
				if (steps and steps["BlockQs"]) then
		...
		local arg1, arg2, arg3, arg4 = ...;
		local ActiveQs = C_GossipInfo.GetActiveQs()
		local ActiveQNr = C_GossipInfo.GetNumActiveQs()
		local CLi
		local NumAvailableQs = C_GossipInfo.GetNumAvailableQs()
		local AvailableQs = {C_GossipInfo.GetAvailableQs()}
		if (ActiveQs and APR1[APR.Realm][APR.Name]["Settings"]["AutoHandIn"] == 1 and not IsControlKeyDown()) then
			for CLi = 1, ActiveQNr do
				if (ActiveQs[CLi] and ActiveQs[CLi]["isComplete"] == true) then
					C_GossipInfo.SelectActiveQ(CLi)
				end
			end
		end
		if (NumAvailableQs > 0 and APR1[APR.Realm][APR.Name]["Settings"]["AutoAccept"] == 1 and not IsControlKeyDown()) then
			if (steps and steps["BlockQs"]) then
			elseif (steps and steps["SpecialPickupOrder"]) then
				C_GossipInfo.SelectAvailableQ(2)
			else
				C_GossipInfo.SelectAvailableQ(1)
			end
		end
	end
	...
	if (event=="Q_DETAIL") then
		...
		if (GetQID() and (APR1[APR.Realm][APR.Name]["Settings"]["AutoAccept"] == 1) and (not IsControlKeyDown())) then
			if (QGetAutoAccept()) then
				CloseQ()
			else
				QInfoDescriptionText:SetAlphaGradient(1, 1)
				QInfoDescriptionText:SetAlpha(1)
				APR.BookingList["AcceptQ"] = 1
			end
		end
	end
	if (event=="Q_PROGRESS") then
		if (APR1["Debug"]) then
			print("Q_PROGRESS")
		end
		local CurStep = APR1[APR.Realm][APR.Name][APR.ActiveMap]
...
expacID, setID, isCraftingReagent
    = GetItemInfo(item)
				...
				local isweaponz = 0
				local APRColorof = 0
				for h=1, GetNumQChoices() do
					local _, _, itemQuality, _, _, _, _, _, itemEquipLoc = GetItemInfo(GetQItemLink("choice", h))
					local ilvl = GetDetailedItemLevelInfo(GetQItemLink("choice", h))
					if (itemEquipLoc == "INVTYPE_WEAPONOFFHAND") then
						itemEquipLoc = "INVTYPE_WEAPON"
					end
					if (itemEquipLoc == "INVTYPE_WEAPONMAINHAND") then
	...
	if (event=="UPDATE_MOUSEOVER_UNIT") then
		...
		if (CurStep and APR.QStepList and APR.QStepList[APR.ActiveMap] and APR.QStepList[APR.ActiveMap][CurStep]) then
			...
			elseif (steps and steps["DroppableQ"]) then
				if (UnitGUID("mouseover") and UnitName("mouseover")) then
					local guid, name = UnitGUID("mouseover"), UnitName("mouseover")
					local type, zero, server_id, instance_id, zone_uid, npc_id, spawn_uid = strsplit("-",guid);
					if (type == "Creature" and npc_id and name and steps["DroppableQ"]["MobId"] == tonumber(npc_id)) then
