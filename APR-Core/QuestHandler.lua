...
function APR.ChkBreadcrums(qids)
	if (qids and APR.Breadcrums and APR.Breadcrums[qids]) then
		for APR_index,APR_value in pairs(APR.Breadcrums[qids]) do
			if ((APR.ActiveQs[APR_value] or C_QuestLog.IsQuestFlaggedCompleted(APR_value) == true) and (not APR.ActiveQs[qids])) then
...
local function APR_QLeave(QIDs)
	C_QuestLog.SetSelectedQ(QIDs)
	C_QuestLog.AbandonQ()
end
...
local function APR_QAskPopWanted()
	...
	elseif (steps["QLineSkip"] and APR1[APR.Realm][APR.Name]["QuestlineSkip"][steps["QLineSkip"]] and APR1[APR.Realm][APR.Name]["QuestlineSkip"][steps["QLineSkip"]] == 0) then
		APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
		APR.BookingList["PrintQStep"] = 1
	else
...
local function APR_PrintQStep()
	...
	if (APR.QStepList and APR.QStepList[APR.ActiveMap] and APR.QStepList[APR.ActiveMap][CurStep]) then
		...
		if (steps and steps["LoaPick"] and steps["LoaPick"] == 123 and ((APR.ActiveQs[47440] or C_QuestLog.IsQuestFlaggedCompleted(47440)) or (APR.ActiveQs[47439] or C_QuestLog.IsQuestFlaggedCompleted(47439)))) then
			APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
			APR.BookingList["PrintQStep"] = 1
			return
		elseif (steps["PickedLoa"] and steps["PickedLoa"] == 2 and (APR.ActiveQs[47440] or C_QuestLog.IsQuestFlaggedCompleted(47440))) then
			APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
			APR.BookingList["PrintQStep"] = 1
			if (APR1["Debug"]) then
			...
			return
		elseif (steps["PickedLoa"] and steps["PickedLoa"] == 1 and (APR.ActiveQs[47439] or C_QuestLog.IsQuestFlaggedCompleted(47439))) then
			APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
			APR.BookingList["PrintQStep"] = 1
			if (APR1["Debug"]) then
		...
		if (steps["QLeave"]) then
			APR_QLeave(steps["QLeave"])
		end
		if (steps["QLeaves"]) then
			for APR_index,APR_value in pairs(steps["QLeaves"]) do
				APR_QLeave(APR_value)
			end
		end
		if (APR1["Debug"]) then
			print(StepP)
		...
		if (StepP == "QPart") then
			...
			if (steps["QPartDB"]) then
				...
				for hz=1, getn(steps["QPartDB"]) do
					...
					if (C_QuestLog.IsQuestFlaggedCompleted(ZeQID) or APR.ActiveQs[ZeQID]) then
						ZeIDi = ZeQID
						break
					end
				end
				...
				for APR_index2,APR_value2 in pairs(APR_value) do
					...
					elseif (APR.ActiveQs[qid]) then
						if (APR1[APR.Realm][APR.Name]["Settings"]["ShowQList"] == 1 and APR.ZoneTransfer == 0) then
							...
							local checkpbar = C_QuestLog.GetQObjectives(APR_index)
		...
		elseif (StepP == "PickUp") then
			...
			if (steps["PickUpDB"]) then
				...
				for h=1, getn(IdList) do
					...
					Total = Total + 1
					if (not APR.ActiveQs[theqid] and C_QuestLog.IsQuestFlaggedCompleted(theqid) == false) then
						NrLeft = NrLeft + 1
					end
					if (C_QuestLog.IsQuestFlaggedCompleted(theqid) or APR.ActiveQs[theqid] or APR.BreadCrumSkips[theqid]) then
						Flagged = Flagged + 1
					end
				end
...
--					APR.QList.QFrames["FS"..LineNr]["Button"]:Hide()
		...
		elseif (StepP == "QDrop") then
			IdList = steps["QDrop"]
			if (C_QuestLog.IsQuestFlaggedCompleted(IdList) or APR.ActiveQs[IdList]) then
				APR1[APR.Realm][APR.Name][APR.ActiveMap] = APR1[APR.Realm][APR.Name][APR.ActiveMap] + 1
				if (APR1["Debug"]) then
					print("APR.PrintQStep:QDrop:Plus:"..APR1[APR.Realm][APR.Name][APR.ActiveMap])
				end
				APR.BookingList["UpdateQ"] = 1
				APR.BookingList["PrintQStep"] = 1
			end
		...
		if (steps["DroppableQ"] and not C_QuestLog.IsQuestFlaggedCompleted(steps["DroppableQ"]["QID"]) and not APR.ActiveQs[steps["DroppableQ"]["QID"]]) then
			if (APR1[APR.Realm][APR.Name]["Settings"]["ShowQList"] == 1 and APR.ZoneTransfer == 0) then
				LineNr = LineNr + 1
	...
	while C_QuestLog.GetTitleForLogIndex(i) do
		...
		if (ZeInfoz) then
			...
			if (QID > 0) then
				...
				local QTitle = C_QuestLog.GetTitleForQID(QID)
				local isComplete = C_QuestLog.IsComplete(QID)
				if (not isHeader) then
					APRQNames[QID] = QTitle
					local numObjectives = C_QuestLog.GetNumQObjectives(QID)
					...
					if (numObjectives == 0) then
						...
						local ZeObject = C_QuestLog.GetQObjectives(QID)
...
local function APR_RemoveQ(QID)
	...
	if (StepP == "Done") then
		local NrLeft = 0
		for APR_index,APR_value in pairs(IdList) do
			if (C_QuestLog.IsQuestFlaggedCompleted(APR_value) or QID == APR_value) then
			else
				NrLeft = NrLeft + 1
			end
		end
...
local function APR_CheckZonePick()
	if (APR.ActiveMap == 862) then
		if (C_QuestLog.IsQuestFlaggedCompleted(50963) == false and (APR.ActiveQs[47514] or C_QuestLog.IsQuestFlaggedCompleted(47514) == true)) then
			APR.BookingList["UpdateMapId"] = 1
			APR.BookingList["PrintQStep"] = 1
		elseif ((APR.ActiveQs[47513] or C_QuestLog.IsQuestFlaggedCompleted(47513) == true) and C_QuestLog.IsQuestFlaggedCompleted(47315) == false) then
			APR.BookingList["UpdateMapId"] = 1
			APR.BookingList["PrintQStep"] = 1
		elseif ((APR.ActiveQs[47512] or C_QuestLog.IsQuestFlaggedCompleted(47512) == true) and C_QuestLog.IsQuestFlaggedCompleted(47105) == false) then
			APR.BookingList["UpdateMapId"] = 1
			APR.BookingList["PrintQStep"] = 1
