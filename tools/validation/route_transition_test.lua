...
APR = { Color = {}, PlayerID = "player", PreviousMaxLvl = 80, MaxLevelChromie = 70,
    EXPANSIONS = { Midnight = "Midnight", TheWarWithin = "TWW" }, PREFAB_TYPES = { Speedrun = "Speedrun" } }
...
assert(APR:FormatLocalizedText("LEAVE_SCENARIO", "Leave %s", "100% fini") == "Quitter 100% fini")
for _, template in ipairs({ "Quitter %S", "Quitter %1$s", "Quitter %", "Quitter %d" }) do
    L.LEAVE_SCENARIO = template
    assert(APR:FormatLocalizedText("LEAVE_SCENARIO", "Leave %s", "Gouffre") == "Leave Gouffre")
end
assert(debugCount == 1, "Repeated bad translations should not flood debug output")
assert(APR:FormatLocalizedText("MISSING", "Leave %s", "Gouffre") == "Leave Gouffre")
...
APR.questionDialog = { CreateQuestionPopup = function(_, id, text, accept)
...
popup.accept()
assert(selected == "TWW")
for _, level in ipairs({ 79, 81, 90 }) do check(level, {}); assert(not popup) end
