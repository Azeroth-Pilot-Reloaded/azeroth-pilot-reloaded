...
- Use `IsQuestReadyForTurnIn` to avoid activating a hand-in for an absent or unfinished quest. Account completion does not make a character's quest ready.
...
| `LootItems`       | Collection objective: displays progress and waits until every entry reaches its `quantity` (default 1). Counts bags/bank and virtual quest loot; supports remembered completion. Optional `questID` also completes the entry when that quest is completed. Unlike `ItemCount`, this is an action, not an eligibility filter. | `LootItems = { { questID = 86644, itemID = 244143, quantity = 1 } }` |
...
| `IsQuestReadyForTurnIn`             | Requires a quest to be in this character's log, with its objectives complete, and not already turned in.                                                                | `IsQuestReadyForTurnIn = 93384`                                                                    |
...
Forever Speedrun delegates to the Forever Leveling builder, so both produce the
same ordered route list. Generated prefab conditions follow the standard 1x
source continuations for each character. AoE, advanced and 2x variants remain
available for manual selection, and missing continuations are not invented.
